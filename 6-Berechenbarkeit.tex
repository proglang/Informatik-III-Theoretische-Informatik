...
\begin{Def}[Reduktion]\label{def:reduktion}Seien $U, V \subseteq \Sigma^*$ Sprachen.
  ...
  und schreiben $U \preceq V$ wenn $U$ auf $V$ reduzierbar ist.
  \datenote{25.01.2019}
\end{Def}
...
\begin{Def}[name={[Universelles Halteproblem $H_{\forall w}$]}]
	...
	$$
	H_{\forall w} = \{u\in\{0,1\}^* \mid \text{$\M_u$ angesetzt auf jedes beliebige Wort $w \in \Sigma^*$ hält}\}.
	$$
\end{Def}
...
\begin{figure}[H]
	\centering
	\begin{tabular}{l@{}cc}
		\hspace{20mm} Sprache & \begin{tabular}{c} ist\\ semi-entscheidbar\end{tabular} & \begin{tabular}{c}Komplement ist\\ semi-entscheidbar\end{tabular}\\ \hline
		Spezielles Halteproblem $K$ & \checkmark & -\\
		Allgemeines Halteproblem $H$ & \checkmark & -\\
		Halteproblem auf dem leeren Band $H_\varepsilon$ & \checkmark & -\\
		Universelles Halteproblem $H_{\forall w}$ & - & -\\
	\end{tabular}
	\caption{Semi-Entscheidbarkeit der in diesem Kapitel vorgestellten unentscheidbaren Sprachen}
\end{figure}
