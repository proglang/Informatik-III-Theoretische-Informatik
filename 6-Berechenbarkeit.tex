...
Die Abbildung $\functopower:\calP(\Sigma^*\rightarrow\Sigma^*)\rightarrow \calP(\N)$
...
\begin{proof}
 ...
 Dann gibt es für jede Funktion $f$ eine TM $\M_L$ mit $f_\M=f$ und somit eine surjektive Abbildung $f:\tmach\rightarrow\calP(\Sigma^*)$.
 Nach \autoref{satz:nattotm} und \autoref{satz:functopower} wäre dann aber auch die Abbildung $\functopower\circ f \circ\nattotm:\N\rightarrow\calP(\N)$ surjektiv.
...
\node[circle, draw=lightgray!50, fill=lightgray!50, inner sep=0] (jj) at (6,-6) {Y};
\node (kj) at (6,-4) {Y};
\draw[->] (jj) to (kj);
