...
\begin{Def}[Reduktion]\label{def:reduktion}Seien $U, V \subseteq \Sigma^*$ Sprachen.
  ...
  \]
  gilt. \datenote{25.01.2019}
  Wir nennen die zugehörige binäre Relation auf Sprachen die \emph{Reduktionsrelation}
...
% 		\node[above=2mm of rR] {$\Sigma^*$};
		...
		\draw[t] (w6) to (f5);
	\end{tikzpicture}
	\caption{Schematische Reduktion $U \preceq V$ von einer Sprache $U$ auf eine Sprache $V$}
\end{figure}
...
\begin{proof}
	Wir zeigen $K \preceq H$. Betrache die Funktion $f(w)=w\# w$. Es gilt:
	\[
	w \in K \text{\ \ gdw. \ \ $\M_w$ hält bei Eingabe $w$ an \ \ gdw.\ \ } \underbrace{w\#w}_{f(w)} \in H 
	\]
	
	Außerdem ist $f$ total und berechenbar.\qedhere
\end{proof}
