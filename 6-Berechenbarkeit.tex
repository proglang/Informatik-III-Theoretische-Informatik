...
\begin{Def}
 Sei $w\in\{0,1\}$. Die \emph{durch $w$ codierte \ac{TM}} $\M_w$ ist wie folgt definiert.
 $$\M_w=\begin{cases}
...
\begin{Bemerkung}
 Im folgenden genügt es wenn wir uns auf \ac{TM}s aus $\tmach$ einschränken. Gibt es eine \ac{TM} die eine Funktion $f$ berechnet, so gibt es auch eine \ac{TM} aus $\tmach$ ie diese Funktion berechnet.
\end{Bemerkung}
