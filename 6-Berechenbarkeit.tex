...
\begin{proof}
  ...
  \begin{itemize}
   \item Fall 1: $\Omega \in S$
     ...
     Sei $f$ eine Funktion, die aus einem Wort $w$ die entsprechende \ac{TM} $\M_\textsf{Evil}$ konstruiert.
     Die Funktion $f$ ist total und berechenbar.
     Mit dieser Funktion können wir die Reduktion $\overline{H}_\Eps \preceq C_S$ zeigen:
     \begin{align*}
      w \in \overline{H_\Eps} &\iff \M_w \text{ hält \emph{nicht} auf dem leeren Band} \\
      ...
      &\iff M_\text{€} \text{ akzeptiert } \godel{\M_{w, f}} \\
      &\iff f(w) \in C_S
    \end{align*}
    Also könnten wir $\M_\textsf{Evil}$ verwenden, um die Sprache $H_\Eps$ zu entscheiden.
    Widerspruch zu \autoref{satz:HalteproblemLeeresBand}.
    \item Fall 2: $\Omega \notin S$
    
    Analog zu Fall 1 mit Reduktion $H_\Eps \preceq C_S$. \qedhere
  \end{itemize}
\end{proof}
