...
\begin{itemize}
 ...
 \item \emph{überabzählbar}, wenn es keine bijektive Abbildung zwischen $X$ und $\N$ gibt.\qedhere
\end{itemize}
\end{Def}
...
Wir wollen hier das folgende, leicht andere, Vorgehen wählen. Dieses ist weniger Allgemein haber aber etwas leichter nachzuvollziehende Beweise.

\begin{enumerate}
 \item Es gibt für jede Turingmaschine eine natürliche Zahl.
 \item Es gibt für jedes Element der Potenzmenge der natürlichen Zahlen $\calP(\N)$ ein Funktion.
...
Da $f$ surjektiv ist, muss es auch für die im formalen Beweis erwähnt Menge $Z=\{x\in X\mid x\notin f(x)\}\in\calP(X)$ eine Zeile geben.
Diese Zeile -- wir nehmen an, es sei die Zeile für $f(x_k)$ -- enthält nun in Spalte $j$ immer genau das Inverse des Diagonaleintrages $(j, j)$.
...
\begin{Def}[name={[Entscheidbarkeit]}]
    ...
    Eine Sprache $L\subseteq\Sigma^*$ heißt \emph{entscheidbar}, falls es eine TM gibt, die $L$ entscheidet.\footnote{
...
\begin{Def}[Charaktertistische Funktion]
...
\begin{Satz}\label{satz:characFunc}
    ...
    Eine Sprache $L\subseteq\Sigma^*$ ist entscheidbar genau dann wenn die charakteristische Funktion $\chi_l$
    berechenbar ist.
\end{Satz}
...
Wir geben zunächst eine einen kurzen formalen Beweis mit Hilfe eines Diagonalargumentes
und diskutieren dieses anschließend.
\begin{proof}
  Angenommen $\M_\text{€}$ sei eine \ac{TM}, die $K$ entscheidet.
  Dann gibt es auch eine TM $\M_\textsf{evil}$, (siehe \autoref{fig:Mevil})
  die zunächst $\M_\text{€}$ auf ihre Eingabe anwendet.\footnote{
  Der Name $\M_\text{€}$ soll den hohen Wert dieser TM betonen.
  Gäbe es einen Algorithmus zur Terminierungsanalyse könnten wir damit viele Probleme in der Sofwareentwicklung lösen.
  Der Name $\M_\textsf{evil}$ soll suggerieren dass diese TM unsere entsprechenden Hoffnungen zunichte macht.
  }
...
\end{proof}
 ...
 \begin{figure}[H]\centering
  ...
  \end{tikzpicture}
  \caption{Definition der TM $\M_\textsf{evil}$ aus dem Beweis von \autoref{satz:speziellesHalteproblem} mit Hilfe eines Flussdiagramms.
  Die TM $\M_{\chi_K}$ verhält sich wie $\M_\text{€}$ aber statt zu akzeptieren (bzw. nicht zu akzeptieren) schreibt diese 1 (bzw. 0) auf das Band und hält (siehe \autoref{satz:characFunc}).}
  \label{fig:Mevil}
 \end{figure}
...
Wir haben außerdem unendlich viele Zeilen, in Zeile $i$ steht die durch $w_i$ codierte TM.
Aus \autoref{satz:bintotm} folgt also dass jede TM in mindestens einer Zeile vorkommt.
...
Die TM $\M_\text{€}$ deren Existenz wir im Beweis angenommen haben (die für alle $w\in\{0,1\}$ entscheidet ob $\M_w$ angesetzt auf $w$ hält) ist eine TM die alle Diagonaleinträge in unserer Matrix bestimmen kann.
Die daraufhin konstruierte TM $\M_\textsf{evil}$ (siehe \autoref{fig:Mevil}) muss in irgendeiner Zeile vorkommen.
Sei $w_k$ das Wort dass die TM $\M_\textsf{evil}$ codiert.
...
\end{tikzpicture}
	\caption{Illustration des Diagonalargumentes beim Beweis der Unentscheidbarkeit des speziellen Halteproblems}
\end{figure}
...
\end{Korollar}
\begin{proof}
	Angenommen $\overline{K}$ sei entscheidbar durch eine TM $\M$. 
	Betrachte TM $\M'$ die zunächst $\M$ ausführt und dann das Ergebnis negiert.
...
\begin{Def}[name={[Semi-Entscheidbarkeit]}]
	Eine Sprache $L\subseteq\Sigma^*$ heißt \emph{semi-entscheidbar}, falls es eine TM gibt, die $L$ akzeptiert.\footnote{
...
\end{lemma}
\begin{proof}
  Die wie folgt konstruierte TM $\M$ akzeptiert $K$.
  Sei $w$ die Eingabe. 
...
\begin{proof}
	...
	Falls $\M$ akzeptiert -- halte und akzeptiere.\\
	Falls $\overline{\M}$ akzeptiert -- halte aber akzeptiere nicht.\\
\end{proof}
...
\end{lemma}
\begin{proof}
  Sei $\M_V$ \ac{TM} die ein (Semi-)Entscheidungsverfahren für $V$ implementiert, 
  sei $\M_f$ \ac{TM} die $f$ berechnet.
...
\end{Satz}
\begin{proof}
  Die wie folgt konstruierte TM $\M$ akzeptiert $H$.
...
\begin{proof}
  ...
  Funktion $f(w\#x) = \godel{\M}$, wobei $\M$ \ac{TM}, die
  \begin{itemize}
