...
% 		Sei $\mathcal{G} = (N,\Sigma,P,S)$ Grammatik für $L\setminus \Eps$ in \ac{CNF}. \\
% 		Def. \ac{NPDA} M durch
% 		\begin{alignat*}{2}
% 			Q &= \{q\} &\quad(=&\text{ Startzustand})\\
% 			\Gamma &= \Sigma\dotcup N\\
% 			Z_0 &= S\\
% 			\delta(q,a,a) &= \{(q,\Eps)\} & a&\in\Sigma\\
% 			\delta(q,\Eps,A) &= \{(q,\alpha)\} & A&\->\alpha\in P
% 		\end{alignat*}
% 		\begin{alignat*}{2}
% 		\text{f"ur alle } X&\in\Sigma\cup N \ ,\ v\in\Sigma^*\\ 
% 			\text{Zeige }&X\overset{*}{\==>} v &
% 			\curvearrowleftright\quad& \mathrlap{(q,v,X) \vdash^{\!\!*} (q,\Eps,\Eps)}
% 		\end{alignat*}
...
% 	falls $\delta(q,x,Z)\ni (q',\alpha) \quad x\in\Sigma\cup\{\Eps\}$
% 	dann ist entweder
% 	\begin{itemize}
% 		\item $\alpha = \Eps$
% 		\item $\alpha = Z'$
...
% 	\item $\delta: Q\x (\Sigma\cup\{\Eps\})\x \Gamma \-> \mathcal{P}(Q\x \Gamma^*)$\\
% 	$|\delta(q,a,Z)| + |\delta(q,\Eps,Z)| \leq 1 \quad \forall q\in Q,a\in\Sigma,Z\in\Gamma$
