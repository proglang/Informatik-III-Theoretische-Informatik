...
\begin{Def}[name={[Menge der Konfigurationen eines \acs*{NPDA}]}]
  ...
  \begin{displaymath}
                L(M) = \{ w\in\Sigma^* \mid (q_0,w,Z_0) \ConfRel^{\!\!*} (q',\Eps,\Eps) \} \tag*{\qedhere}
  \end{displaymath}
\end{Def}
...
\begin{Bsp*}
  ...
  \begin{array}{ll}
    (q_0, 0110, \bot) \\
    \ConfRel (q_0, 110, 0\bot)  &\text{(,,pushen'' des Eingabesymbols $0$)}\\
    \ConfRel (q_0, 10, 10\bot)  &\text{(,,pushen'' des Eingabesymbols $1$)}\\
    \ConfRel (q_1, 10, 10\bot)  &\text{($\Eps$-Übergang von $q_0$ nach $q_1$)} \\
...
\begin{proof}\hfill
  ...
  \item Wir zeigen
    \begin{displaymath}
      \text{wenn } S \stackrel{*}{\Longrightarrow} w \text{ dann } (q_0, w, S) \ConfRel^* (q_0, \Eps, \Eps)
    \end{displaymath}
    Wir beweisen dazu die stärkere Aussage
    \begin{displaymath}
      \forall A \in N: \text{ wenn } A \stackrel{*}{\Longrightarrow} w\text{ dann } (q_0, w, A) \ConfRel^* (q_0, \Eps, \Eps)
    \end{displaymath}
    ...
    \item[IV] Für alle $n' \le n$ und $1 \le i \le n',\enspace A_i \in N,\enspace \mathcal{A}_i \in \operatorname{Abl}(\mathcal{G}, A_i)$ gilt:
      ...
      (q_0, Y(\mathcal{A}_i), A_i) \ConfRel^* (q_0, \Eps, \Eps) \tag{$\star$}
    \end{equation}
    \item[IS] Unterscheide die Form der Produktionen der CNF-Grammatik $\mathcal{G}$:
      \begin{itemize}
      \item $\pi = S \to \Eps$.
        Per Konstruktion gilt $(q_0, \Eps) \in \delta(q_0, \Eps, S)$ und somit $(q_0, \Eps, S) \ConfRel (q_0, \Eps, \Eps)$.
      \item $\pi = A \to a$, $a \in \Sigma^*$.
        Per Konstruktion gilt $(q_0, a) \in \delta(q_0, \Eps, A)$ und $(q_0, \Eps) \in \delta(q_0, \Eps)$.

        Somit gilt $(q_0, a, A) \ConfRel (q_0, a, a) \ConfRel (q_0, \Eps, \Eps)$.
      \item $\pi = A \to BC$, $B,C \in N$, $\mathcal{A} = \pi(\mathcal{A}_1, \mathcal{A}_2)$, $\mathcal{A}_1 \in \operatorname{Abl}(\mathcal{G}, B)$, $\mathcal{A}_1 \in \operatorname{Abl}(\mathcal{G}, B)$, $w = Y(\mathcal{A}_1)Y(\mathcal{A}_2)$.
        ...
        Ferner gilt per IV, dass $(q_0, Y(\mathcal{A}_1), B) \ConfRel^* (q_0, \Eps, \Eps)$ und $(q_0, Y(\mathcal{A}_2), C) \ConfRel^* (q_0, \Eps, \Eps)$.

        Es folgt mit Beobachtung \eqref{eq:pda-step-base}, dass $(q_0, w, A) = (q_0, Y(\mathcal{A}_1)Y(\mathcal{A}_2), A)\ConfRel (q_0, Y(\mathcal{A}_1)Y(\mathcal{A}_2), BC) \ConfRel^* (q_0, Y(\mathcal{A}_2), C) \ConfRel^* (q_0, \Eps, \Eps)$.
      \end{itemize}
    \end{description}
  \item Wir zeigen
    \begin{displaymath}
      \text{wenn } (q_0, w, S) \ConfRel^* (q_0, \Eps, \Eps) \text{ dann } S \stackrel{*}{\Longrightarrow} w
    \end{displaymath}
    Wir beweisen dazu die stärkere Aussage
    \begin{displaymath}
      \forall n \in \mathbb{N}: \forall w \in \Sigma^*, \alpha \in \Gamma^*: \text{ wenn } (q_0, w, \alpha) \ConfRel^n (q_0, \Eps, \Eps) \text{ dann } \alpha \stackrel{*}{\Longrightarrow} w 
    \end{displaymath}
    ...
    \item[IV] Für alle $n' < n$ und $w \in \Sigma^*, \alpha \in \Gamma^*$ gilt
      \begin{displaymath}
      \text{ wenn } (q_0, w, \alpha) \ConfRel^{n'} (q_0, \Eps, \Eps) \text{ dann } \alpha \stackrel{*}{\Longrightarrow} w
    \end{displaymath}
  \item[IS] $n > 0$, $\alpha = Z\alpha'$,

    $(q_0,w,Z\alpha') \ConfRel (q_0, w', \beta\alpha') \ConfRel^{n-1} (q_0, \Eps, \Eps)$, $\delta(q_0, x, Z) \ni (q_0, \beta)$
    ...
    \item Wir zeigen, dass 
      ...
      \item[IS]
        ...
        \item $(q, Z, q') \to x(q'', Z', q'), \enspace \delta(q, x, Z) \ni (q'', Z'), q'' \in Q, x \in \Sigma \cup \{\Eps\}$
          ...
          Es folgt per IV, dass $(q'', Y(\mathcal{A}_1), (q'', Z', q')) \ConfRel^* (q_0, \Eps, \Eps)$ und damit auch $(q, xY(\mathcal{A}_1), (q, Z, q')) \ConfRel (q'', Y(\mathcal{A}_1), (q'', Z', q')) \ConfRel^* (q_0, \Eps, \Eps)$.
...
\begin{Def}[name={[DPDA]}]
        ...
        \item $L(M) = \{w\in\Sigma^* \mid (q_0,w,Z_0) \ConfRel^* (q',\Eps,\gamma) \land q'\in F \}$ \qedhere
        \end{itemize}
\end{Def}
...
\begin{proof}
  ...
  $\gamma\in\Gamma'^*$ so dass $(q_0', w, Z_0') \ConfRel^* (q,
