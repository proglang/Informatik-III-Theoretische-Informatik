...
Sei $\mathcal{G} = (\Sigma,N,P,S)$ eine kontextfreie Grammatik für $L$ in \ac{CNF}.
                ...
                \begin{itemize}
                        ...
                        \item $\qinit = q$
                        \item $\Zinit = S$
                        \item $\delta(q,a,a) = \{(q,\Eps)\} $ für $ a\in\Sigma$
                        \item $\delta(q,\Eps,A) = \{(q,\alpha)\}$ für $A\to\alpha\in P$
                \end{itemize}
...
\begin{itemize}
 ...
 Wir wollen zeigen dass $(q, w, S) \ConfRel^* (q, \Eps, \Eps)$ folgt und beweisen dafür via vollständige Induktion über die Länge der Ableitung die folgende stärkere Eigenschaft.
     \begin{displaymath}
      \forall n>0 \text{ wenn } A \stackrel{n'}{\vdash} w \text{ für $A \in N$ und $n'\leq n$ \ \ dann \ \ } (q, w, A) \ConfRel^* (q, \Eps, \Eps)
    \end{displaymath}
  I.A.: $n=1$: Es gibt nur zwei Arten von Regeln, die für Ableitungen der Länge 1 in Frage kommen:
  \begin{itemize}
        \item $S \to \Eps$.
        Per Konstruktion gilt $(q, \Eps) \in \delta(q, \Eps, S)$ und somit $(q, \Eps, S) \ConfRel (q, \Eps, \Eps)$.
      \item $A \to a$, $a \in \Sigma$.
        Per Konstruktion gilt $(q, a) \in \delta(q, \Eps, A)$ und $(q, \Eps) \in \delta(q, a, a)$.

        Somit gilt $(q, a, A) \ConfRel (q, a, a) \ConfRel (q, \Eps, \Eps)$.
   \end{itemize}
   ...
   Wir betrachten den zugehörigen Ableitungsbaum $\mathcal{T} = \pi(\mathcal{T}_1, \mathcal{T}_2)$, $\mathcal{T}_1 \in \operatorname{Abl}(B)$, $\mathcal{T}_2 \in \operatorname{Abl}(C)$, $w = Y(\mathcal{T}_1)Y(\mathcal{T}_2)$.
        ...
        Per Konstruktion gilt $(q, BC) \in \delta(q, \Eps, A)$. 

        Ferner gilt per I.V., dass $(q, Y(\mathcal{T}_1), B) \ConfRel^* (q, \Eps, \Eps)$ und $(q, Y(\mathcal{T}_2), C) \ConfRel^* (q, \Eps, \Eps)$.

        Mit \autoref{lem:4.mehrKeller} folgt schließlich:
        \begin{align*}
          (q, w, A) = (q, Y(\mathcal{T}_1)Y(\mathcal{T}_2), A) &\ConfRel (q, Y(\mathcal{T}_1)Y(\mathcal{T}_2), BC) \\
          &\ConfRel^* (q, Y(\mathcal{T}_2), C) \\
          &\ConfRel^* (q, \Eps, \Eps).
        \end{align*}
 \item Beweisrichtung  "`$\subseteq$"'
 
 Sei $w\in L(\K)$, dann gilt $(q, w, S) \ConfRel^* (q, \Eps, \Eps)$.
 Wir wollen $S \stackrel{*}{\vdash} w$ folgern und beweisen dafür via vollständige Induktion über die Anzahl der Berechnungsschritte $n$ die folgende Aussage:
     \begin{displaymath}
      \forall n \in \mathbb{N}: \forall w \in \Sigma^*, \alpha \in \Gamma^*: \text{ wenn } (q, w, \alpha) \ConfRel^n (q, \Eps, \Eps), \text{dann } \alpha \stackrel{*}{\vdash} w
    \end{displaymath}
  ...
  \item[I.S.] $n > 0$, $\alpha = Z\alpha'$: Es gilt
    $(q,w,Z\alpha') \ConfRel (q, w', \beta\alpha') \ConfRel^{n-1} (q, \Eps, \Eps)$, \linebreak
    $\delta(q, x, Z) \ni (q, \beta)$, $x \in \Sigma \cup \{\varepsilon\}$.
