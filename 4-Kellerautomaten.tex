...
\begin{Bemerkung}
Zu jedem NPDA gibt es einen äquivalenten NPDA der nur einen Zustand hat.
...
%      Dann kann man sie aber auch gleich $i$-fach nutzen ($v^i$ und $x^i$), und ist immernoch in der Sprache.}
  $\mathcal{T}$ ist ein Binärbaum mit $|z|\geq n = 2^{|N|}$ Blättern. In einem solchen Binärbaum existiert ein Pfad $\zeta$ der Länge $\geq |N|$.
	Auf $\zeta$ liegen $\geq |N|+1$ Nichtterminalsymbole, es muss also mindestens ein Nichtterminal $A$ mehrmals vorkommen.
  ...
  Teile $z = uvwxy$ wie in \autoref{zerlegung} skizziert. Es gilt:
      ...
      \item $|vx|\geq 1$, da $\zeta$ entweder durch $B$ oder durch $C$ läuft. Nehme also an, $\zeta$ verläuft durch $B$.
        Somit muss $C \stackrel{*}{\vdash} x$. In \ac{CNF} ist $C \stackrel{*}{\vdash} \Eps$ nicht möglich und somit ist $|x| \ge 1$.
        Der Fall, das $\zeta$ durch $C$ verläuft, ist analog.
      \item $|vwx| \le 2^{|N|} = n$, da das obere $A$ höchstens $|N|$ Schritte von Blattebene entfernt und $\mathcal{T}$ Binärbaum ist.
      \item Für $\forall i\in\N: uv^iwx^iy\in L$ unterscheiden wir drei Fälle
      \begin{itemize}
				...
				Wir könnten also auch aus dem unteren $A$ nochmal $vAx$ ableiten und wenn wir dies oft genug wiederholen jedes $uv^iwx^iy$ mit $i\geq 2$ erzeugen.
				(Siehe \autoref{uviwxiy}).
...
\begin{proof}
  ...
  Nach PL gibt es eine Zerlegung $z = uvwxy$ mit $|vx| \ge 1$ und $|vwx| \le n$.
  ...
  \item $vwx = a^j$: 
    ...
    Anzahl der $a$ stimmt nicht mehr mit Anzahl $b$ überein.
  \item $vwx = a^kb^j$:
    \begin{itemize}
    \item Falls $v = a^{k'}$, $x = b^{j'}$: Für $i=0$ stimmt die Anzahl der $a$ oder die Anzahl der $b$ nicht mehr mit Anzahl $c$ überein.
...
\end{Satz}
\begin{proof}
	Für zwei \ac{CFG} $\mathcal{G}_i = (\Sigma, N_i, P_i,S_i), i=1,2$, konstruiere $\mathcal{G}_i = (\Sigma, N,P,S)$ mit
	\begin{align*}
...
\[
	\mathcal{E} = (\Sigma,Q,\Gamma,q_0,\Zinit,\delta,F)
\]
mit den Komponenten $\Sigma,Q,\Gamma,q_0,\Zinit,\delta$ wie für einen gewöhnlichen Kellerautomaten und $F \subseteq Q$ als die Menge der akzeptierenden Zustände.
...
\begin{lemma}
Zu jedem (gewöhnlichen) Kellerautomaten gibt es einen Kellerautomaten mit akzeptierenden Zuständen der die gleiche Sprache akzeptiert und umgekehrt.
\end{lemma}
\begin{proof}
...
\begin{Def}[name={[DPDA]}]
        Ein \ac{DPDA} ist ein 7-Tupel Tupel 
...
\begin{proof}
  ...
  Zwei Möglichkeiten, warum $\mathcal{D}$ nicht die gesamte Eingabe verarbeiten:
...
%   so dass $a \not \Rightarrow^* b$} 
  ...
  \begin{align*}
  Q_S &= Q \cup \{\qinit_S, q_s\} \text{ neue Zustände: $\qinit_S$ neuer Startzustand und $q_s$ Senkzustand} \\
  F_S &= F \\
  ...
  \item Transition totalisieren: Für alle $q\in Q$, $Z \in \Gamma$\\
    \begin{align*}
      ...
      \end{cases}
    \end{align*}
    Intuition: Wenn $\delta(q, a, Z) = \emptyset$ und ein $\epsilon$-Übergang ebenfalls ausscheidet, kann das Wort von $\mathcal{D}$ nicht weiter abgearbeitet werden. $\mathcal{D}_S$ geht nun in Senkzustand $q_s$ und arbeitet dort weiter. Analog für $\delta(q, \epsilon, Z)$.
  ...
  In $q_s$ kann der Automat nun für alle $a\in\Sigma$, $Z\in \Gamma_S$ abarbeiten:\\
    \[\delta_S (q_s, a, Z) = \{ (q_s, Z) \}\]   %TODO warum Z und nicht \Zinit_S ?!
  ...
  $\gamma\in\Gamma_S^*$ so dass $(\qinit_S, w, \Zinit_S) \ConfRel^* (q,
  \varepsilon, \gamma)$
...
\begin{proof}
  ...
  Nach \autoref{lem:DPDA ges. Eingabe} gibt es \ac{DPDA} $\mathcal{D}$ der die komplette Eingabe liest mit $L(\mathcal{D})=L$.

  Konstruiere \ac{DPDA} $\mathcal{D}_K$, s.d. $ L(\mathcal{D}_K) = \overline{L}$. 
  Definiere dazu $Q_K= Q \times \{0,1,2\}$. Bedeutung des zusätzlichen ,,Flags'' $\in \{0,1,2\}$:
  \begin{itemize}
  \item $0$: seit Lesen des letzten Symbols $a \in \Sigma$ wurde kein akzeptierender Zustand durchlaufen
...
\begin{proof}
 Offensichtlich gibt es eine Teilmengenbeziehung, denn die von nichtdeterministischen Kellerautomaten mit akzeptierendem Zustand akzeptierten Sprachen sind gerade die kontextfreien Sprachen und jeder deterministische Kellerautomat in ein Kellerautomaten mit akzeptierendem Zustand.
 Die deterministisch kontextfreien Sprachen sind und Komplement abgeschlossen die kontextfreien Sprachen sind es nicht:
 Beide Mengen von Sprachen können also nicht gleich sein.
\end{proof}
