...
Wir möchten zunächst mit einer Konvention und einer Definition dafür sorgen dass die Codierung einer Turingmaschine eindeutig ist.

\textbf{Konvention: } Wir wollen von nun an annehmen dass das Alphabet $\Sigma=\{a_1,\ldots, a_{n_\Sigma}\}$ ist,
bzw. das es eine eindeutige Reihenfolge der Symbole gibt.

\begin{Def}[name={[normierte \ac{TM}]}]
	...
	\begin{itemize}
		...
		\item Für alle $x\in\Gamma$ existiert ein $q\in Q$ sodass $\delta(q,x)\neq\{\}$.
	\end{itemize}
...
\end{lemma}
\begin{proof}
	Umbenennen und Löschen von Zuständen.
\end{proof}
