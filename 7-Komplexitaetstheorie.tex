...
\begin{enumerate}
 ...
 \item Erzeuge aus $F_\mathsf{NNF}$ eine Formel mit Biimplikationszeichen ``$\<->$'' mit Hilfe der folgenden induktiv definierten Abbildungen $\alpha$ und $\gamma$.
 
 Idee: Die Abbildung $\gamma$ ordner jeder $\land$-Teilformel und jeder $\lor$-Teilformel eine Hilfsvariable zu.
 ...
 Die Abbildung $\alpha$ konstuiert für jede Teilformel die entsprechenden Bedingungen für die Hilfsvariable und stelle einer Konjunktion all dieser Bedingungen zusammen.
 ...
 \begin{cases}
   ...
   \alpha(F_1)\land\alpha(F_2)\land\big(\gamma(F)\<-> \gamma(F_1) \land \gamma(F_2)\big) & \text{ falls } F=F_1\land F_2\\
   \alpha(F_1)\lor\alpha(F_2)\lor\big(\gamma(F)\<-> \gamma(F_1) \lor \gamma(F_2)\big) & \text{ falls } F=F_1\lor F_2
  \end{cases}$$
