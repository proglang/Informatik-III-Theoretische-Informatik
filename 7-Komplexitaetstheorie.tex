...
Wir haben bereits gesehen, dass jede Laufzeitschranke auch eine Beschränkung der Platzkomplexität,
...
\begin{lemma}
 ...
 Dann enthält jede Berechnung der Länge > $s(n)\cdot |Q| \cdot |\Gamma|^{s(n)}$
 mindestens eine Konfiguration doppelt.
\end{lemma}
\begin{Bemerkung}
 Für jede DTM mit Platzkomplexität $s(n)$ gilt also dass jede Berechnung entweder gar nicht oder nach höchstens $s(n)\cdot |Q| \cdot |\Gamma|^{s(n)}$ Schritten hält.
\end{Bemerkung}
...
\end{Satz}
\begin{proof}
 Simuliere jede Berechnung bis zu $s(n)\cdot |Q| \cdot |\Gamma|^{s(n)}$ Schritten.
\end{proof}
...
\begin{proof}
 Sei $w\in\Sigma^*$ und $n$ die Länge von $w$. 
 Betrachte alle Ableitungen 
 $$S\vdash\alpha_1\vdash \ldots \vdash\alpha_n,$$
 bei denen alle $\alpha_i$ paarweise verschieden sind und Länge $\leq n$ haben.
\end{proof}
