...
\smallskip
\textit{Gegeben:} Eine Menge $M$ und eine Menge von Teilmengen $\mathcal{S}$ (d.h. $\mathcal{S}=\{S_1,\dots,S_n\}$, $S_i\subseteq M$ für $i=1,
\dots, n$) sowie eine natürliche Zahl $k\leq n$.

\medskip

\textit{Frage:} Gibt es eine Teilmenge $T\subseteq M$, sodass $|T|\leq k$ und $T\cap S_i\neq\varnothing$ für $i=1,\dots n$? (Mit anderen Worten: Gibt es eine höchstens $k$-elementige Teilmenge $T$, die mit jedem $S_i$ mindestens ein gemeinsames Element hat?)
}}
\end{center}
