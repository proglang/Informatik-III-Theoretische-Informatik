...
\begin{proof}\
	...
	\item \ac{SAT} ist \ac{NP}-hart.\\
		Zeige: $\forall L\in \ac{NP}: L \preceq_p \ac{SAT}$\\
		$L\in \ac{NP}: \exists p$ Polynom, \ac{NTM} $M$ mit $L=L(M)$ mit Zeitschranke $T_M(w)\leq p(|w|)$.
...
\begin{proof}
	...
	\item Endkonfiguration
		...
		$|F|$ ist polznomiell beschränkt in $|M,w|$, also $L\preceq_p \ac{SAT}$\\
