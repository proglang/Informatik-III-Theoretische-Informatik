\section{Grammatiken und kontextfreie Sprachen}\datenote{02.06.2026}
...
\begin{Def}
	...
	\begin{itemize}
	\item $\Sigma$ ist ein Alphabet, dessen Elemente wir \emph{Terminalsymbole} nennen.
	\item $N$ ist eine endliche Menge, $N\cap\Sigma = \emptyset$, deren Elemente wir \emph{Nichtterminalsymbole} oder \emph{Variablen} nennen. 
	...
	\item $S\in N$ ist ein Nichtterminalsymbol, das \emph{Startsymbol}.
	\qedhere
	\end{itemize}
\end{Def}
...
\begin{Def}[Ableitungsrelation, Ableitung, Sprache einer Grammatik]
	...
	zu $\mathcal{G}=(\Sigma,N,P,S)$ ist 
        $ \cdot \vdash_\mathcal{G} \cdot \subseteq
        (N\cup\Sigma)^*\x(N\cup\Sigma)^* $
        definiert durch
        \begin{gather*}
          \frac{\alpha \to \beta \in P}{\gamma\cdot\alpha\cdot\delta \vdash_\mathcal{G} \gamma\cdot\beta\cdot\delta }
        \end{gather*}
    
  % mit \ \ $\alpha \vdash_\mathcal{G} \beta$\ \  gdw\ \  $\alpha =
  % \gamma_1\alpha'\gamma_2$,\ \  $\beta = \gamma_1\beta'\gamma_2$\ \
  % und \ \ $\alpha' \to \beta' \in P$ 
  ...
  Die \emph{Ableitung von $\beta$ aus $\alpha$}, geschrieben $\alpha
  \vdash^*_\mathcal{G} \beta$ ist induktiv definiert als
  die reflexive, transitive Hülle von "`$\vdash_\mathcal{G}$"':
  \begin{align*}
    \frac{}{\alpha \vdash^*_\mathcal{G} \alpha}
    &
    &
      \frac{{\alpha \vdash_\mathcal{G} \beta} \quad \beta
      \vdash^*_\mathcal{G} \gamma}
      {{\alpha \vdash^*_\mathcal{G} \gamma}}
  \end{align*}
  Falls $\alpha \vdash^*_\mathcal{G} \beta$, so ist $\beta$ eine
  \emph{Satzform von $\mathcal{G}$}. Oft wird die Ableitung durch eine
  Liste von Satzformen $\alpha_0,\ldots,\alpha_n$ angegeben mit
  $\alpha_i \vdash_\mathcal{G} \alpha_{i+1}$ für $0 \le i < n$.
  

  Ein Wort $w\in\Sigma^*$ wird von $\mathcal{G}$ \emph{erzeugt}, wenn $S \vdash^*_{\mathcal{G}} w$ gilt.
	Die von $\mathcal{G}$ \emph{erzeugte Sprache} ist definiert als:
	\[ L(\mathcal{G}) = \{w\in\Sigma^* \mid S \vdash^*_{\mathcal{G}} w \}  \]
	
	Wir nennen zwei Grammatiken \emph{äquivalent}, falls sie die gleiche Sprache erzeugen.
\end{Def}
...
Es gilt: $L(\mathcal{G})$ ist die Sprache der Wörter über $\{0,1\}$, die gleich viele Nullen wie Einsen haben:
  ...
  Dass $L(\mathcal{G}) \subseteq L$, lässt sich mittels Induktion über
  die Ableitung $S \vdash^*_\mathcal{G} w$ zeigen. 
  Der Beweis wird als Übung dem Leser überlassen.

  Wir zeigen $L \subseteq L(\mathcal{G})$, d.h. $\forall w\in
  \Sigma^*. w \in L \=> w \in L(\mathcal{G})$.
  Hierzu definieren wir eine Hilfsfunktion $d : \Sigma^* \to \Z$:
  \begin{align*}
    d(\Eps) &= 0 &
    d(1w) &= d(w) + 1 &
    d(0w) &= d(w) - 1
  \end{align*}
  Mittels Induktion über $u$ zeige, dass $u \in L \<=>
  d(u) = 0$ und $d(u \cdot v) = d(u) + d(v)$. 
  
  Wir betrachten nun die folgende Aussage
  \begin{align*}
    P(n) &= \forall w.\, |w| = n \=> w \in L \=> w \in L(\mathcal{G})
           \text.
  \end{align*}
  und beweisen sie mit Hilfe des Beweisprinzips der vollständigen
  (bzw.\ starken) Induktion:
  \begin{gather*}
    \frac{
      \forall n.\, (\forall j.\, j<n \=> P(j)) \=> P(n)
    }{\forall n.\, P (n)}
  \end{gather*}
  Die Ausformulierung des Beweises erfolgt in der Übung.
\hide{  
Fallunterscheidung auf $n$.
\begin{itemize}[font=\normalfont]
\item $n = 0$: $w = \Eps$. Es gilt $\Eps \in L \cap  L(\mathcal{G})$, da $\#_0(\Eps) = \#_1(\Eps) = 0$.
\item $n > 0$: Angenommen $w\in L$,  $|w| > 0$, $w = aw'$, $a \in \{0,1\}$.
  ...
  Wir zeigen zunächst, dass wir $w'$ in $u1v$ mit $d(u) = 0$ und $d(v) = 0$ zerlegen können:
%   \begin{itemize}
%   \item[] 

    Sei $w' = a_2 \ldots a_n$.
    Betrachte die Folge $d_1,\ldots,d_n$ mit $d_1 = 0$ und $d_i = d(a_2\ldots a_i)$ für $1 < i \leq n$.
    Wähle $0 < i < n$ maximal, sodass für alle $0 < j \le i$ gilt: $d_j < 1$.%
    \footnote{Wir wissen, dass $i < n$, da $d_n = d(w') = 1$.}

    Da $i$ maximal ist, folgt $d_{i+1} \ge 1$.
    Da $|d_{j+1} - d_j| = 1$ für alle $0 \le j \le i$, folgt $d_{i+1}
    - d_i = 1$ und damit auch $d_i = 0$, $d_{i+1} = 1$ und $a_{i+1} =
    1$. 
    Setze $u = a_1\ldots a_{i}$.

    Es gilt also $w' = a_1\ldots a_{i}a_{i+1}v = u1v$ mit $d(u) = 0$.

    Da $d(u \cdot v) = d(u) + d(v)$ und $d(w') = d(u1v) = 1$, folgt $d(v) = 0$.
%   \end{itemize}
  \bigskip
  \goodbreak

  Da $|u| < n$ und $|v| < n$, folgt nach I.V., dass $S
  \vdash^*_{\mathcal{G}} u$ und $S \vdash^*_{\mathcal{G}} v$. 

  Daraus konstruieren wir die Ableitung
  $S \vdash_{\mathcal{G}} 0S1S \vdash^*_{\mathcal{G}} 0u1S \vdash^*_{\mathcal{G}} 0u1v$.
  \qedhere
\end{itemize}
}
\end{Bsp}
...
\begin{Def}[Chomsky-Hierarchie]\
	...
	\begin{alignat*}{2}
		&A\->w &\quad w&\in\Sigma^*,\, A \in N\\
		\text{oder}\quad &A\->aB& a&\in\Sigma,\, A,B\in N
	\end{alignat*}
	\end{itemize}
...
\subsection{Kontextfreie Sprachen}\datenote{skipped}
...
\datenote{03.06.2026}
\begin{Def}[Ableitungsbaum] Sei $\mathcal{G} = (\Sigma, N, P, S)$ eine
  kontextfreie Grammatik und $A\in N$.
  Wir definieren die Menge der \emph{in $A$ beginnenden Ableitungsbäume von $\mathcal{G}$}, $\operatorname{Abl}(A)$, als Menge von beschrifteten, geordneten Bäumen induktiv wie folgt:
  
  Falls $\pi = A \to w_0A_1w_1\ldots A_nw_n \in P$ mit $A_i \in N$, $ w_i \in \Sigma^*$, $0 \le i \le n$ und $\T_i \in \operatorname{Abl}(A_i)$, dann ist
    \begin{center}
    ...
    Das \emph{abgeleitete Wort zu einem $\T \in
      \operatorname{Abl}(A)$}, $Y : \operatorname{Abl}(A) \to \Sigma^*$\footnote{Wir verwenden $Y(\T)$,
      um an das englische Wort "`yield"' zu erinnern.}, ist definiert
    durch 
    \begin{displaymath}
      Y\left(\tikz[baseline=-0.5cm]{\Tree [.$\pi$ $\T_1$ \edge[draw=none]; {$\dots$} $\T_n$ ]} \right) = w_0Y(\T_1)w_1\ldots Y(\T_n)w_n
    \end{displaymath}
    wobei $\pi = A \to w_0A_1w_1\ldots A_nw_n \in P$.
\end{Def}

\begin{Bemerkung}
Nach dieser Definition gibt es für jede Regel $\pi$,
bei der nur Terminalsymbole auf der rechten Seite vorkommen, einen
Ableitungsbaum, der die ein-elementige Knotenmenge $\{\pi\}$ und eine
leere Menge von Kanten hat. 
\end{Bemerkung}
...
\begin{Def}[Eindeutigkeit von \acs*{CFG} und \acs*{CFL}]\
	\begin{itemize}
...
Wir lernen in diesem Unterkapitel eine spezielle Form von
kontextfreien Grammatiken (Chomsky-Normalform, CNF) kennen, für die gilt:
\begin{itemize}
\item Es gibt einen einfachen Algorithms für das Wortproblem ($w\in
  L(\mathcal{G})$?). 
\item Jede kontextfreie Grammatik lässt sich effektiv in CNF transformieren. 
\end{itemize}

 \begin{Def}[\acs*{CFG} in \acs*{CNF}]
...
\begin{Def}
  Eine \ac{CFG} heißt \emph{separiert}, wenn jede Regel eine der folgenden Formen hat.
  \begin{alignat*}{3}
...
\begin{itemize}
	\item Laufe einmal über alle Regeln und ersetze jeweils $a
          \mapsto Y_a$.
...
\begin{proof}
  ...
  \begin{align*}
    M_0 &= \emptyset \\
    M_{i+1} &= \{ A \mid A \to \alpha \in P \text{ und }
              \alpha \in M_i^* \}
  \end{align*}
  Es gilt für alle $i \in \N$, dass $M_i \subseteq M_{i+1} \subseteq N$.
  Da $|N|$ endlich ist, existiert ein $m \in \N$, sodass
  \begin{displaymath}
    M_m = M_{m+1} = \bigcup_{i \in \N} M_i
  \end{displaymath}
  Wir können daher $\bigcup_{i \in \N} M_i$ mit dem Algorithmus in
  Abb.~\ref{fig:algorithm-nullable} in $O(|\mathcal{G}|^3)$ berechnen. 
...
repeat
    changed = false
    $M_{old} = M$
    foreach $A \to \alpha \in P$:
         if ($ A \not \in M \wedge \alpha \in M_{old}^*$):
             $M = M \cup \{A\}$
             changed = true
until not changed
return M
\end{lstlisting}
\end{minipage}
\caption{Berechung von $\operatorname{Nullable} (\G)$}
\label{fig:algorithm-nullable}
\end{figure}
  ...
  "`$\supseteq$"'\quad
  Wir zeigen mittels Induktion, dass 
  ...
  \item[IS] $i \rightsquigarrow i+1$
    ...
    existiert $A \to A_1\ldots A_n \in P$ mit $A_j \in M_i$ für alle
      $1 \le j \le n$.
      Nach IV existieren Ableitungsbäume $\T_j$ für
      $A_j \vdash^*_\mathcal{G} \Eps$,
      die zu einem Ableitungsbaum von $A$ nach $\Eps$ kombiniert
      werden können:
      \begin{displaymath}
        \T = \left\{\tikz[baseline=-0.5cm]{\Tree [.{$A  \to A_1 \ldots A_n$} $\T_1$ \edge[draw=none]; {$\dots$} $\T_n$ ]}\right.
      \end{displaymath}
      % \begin{displaymath}
      %   A  \vdash_\mathcal{G} A_1 \ldots A_n \vdash^*_\mathcal{G} \underbrace{\Eps \ldots \Eps}_{n \text{ Mal}} = \Eps
      % \end{displaymath}
      Wegen $Y(\T_j) = \Eps$, $1\le j \le n$, gilt auch $Y(\T) = \Eps$.
  ...
  % \in \N$, sodass $A \vdash^*_\mathcal{G} \Eps$ mit einem
  % Ableitungsbaum der Höhe $m$.

  Wir zeigen mittels Induktion über einen Ableitungsbaum von $A
  \vdash^*_\mathcal{G} \Eps$, dass $\exists i.\, A \in M_i$. 
  \begin{description}
  \item[IA] $A \to \Eps$ ist eine Produktion. Also ist
    $A \in M_1$.
  \item[IS] Die Wurzel des Ableitungsbaums ist mit $A \to A_1\dots
    A_n$ beschriftet und die Teilbäume entsprechen den Ableitungen
    $A_j \vdash^*_\mathcal{G} \Eps$, $1\le j \le n$.

    Die IV liefert für jeden Teilbaum ein $i_j$, sodass $A_j \in
    M_{i_j}$.
    Also gilt für $i = \max \{i_1, \dots, i_n \}$, dass $A_j \in M_i$.
    Daher ist $A \in M_{i+1}$.
    \qedhere
  \end{description}

  % Wir zeigen mittels Induktion über $m$, dass $A \in M_{m+1} \subseteq \bigcup_{i \in \N} M_i$.
  % \begin{description}
...
\datenote{09.06.2026}
\begin{Korollar}
  Man kann zu einer \ac{CFG} $\mathcal{G}$ entscheiden, ob $\Eps \in
  L(\mathcal{G})$.
\end{Korollar}
\begin{proof}
...
\begin{proof}
  \hfill
  \begin{enumerate}
  \item Definiere $\G'$ wie folgt:
    \begin{itemize}
    \item Erweitere $\mathcal{G}$ um ein neues Startsymbol
      $S' \notin N$ mit $S' \to S$ als neue Regel.  Dieser Schritt
      stellt sicher, dass $S'$ auf keiner rechten Regelseite vorkommt.
    \item Wende erst \textsc{Sep}, dann \textsc{Bin} an.
      Nun hat jede rechte Regelseite von $\G'$ die Form $a \in \Sigma$ oder $\Eps$ oder $A$ oder $BC$.
    \end{itemize}
  \item Für alle Regeln $A \to BC \in P$:
    \begin{itemize}
    \item Falls $B \in \operatorname{Nullable}(\G')$ füge $A \to C$ hinzu.
    \item Falls $C \in \operatorname{Nullable}(\G')$ füge $A \to B$ hinzu.
    \end{itemize}
  \item Falls $S \in \operatorname{Nullable}(\G')$, füge $S' \to \Eps$ hinzu.
...
\begin{Def}
  ...
  Eine Regel der Form $A \to B \in P$ mit $A, B \in N$ heißt \emph{Kettenregel}.
\end{Def}

\begin{lemma}[\textsc{Unit}]
Es gibt einen Algorithmus der für eine \ac{CFG} $\mathcal{G}$ in
$O(|\mathcal{G}|^3)$ eine äquivalente \ac{CFG} $\mathcal{G}'$ ohne
Kettenregeln mit $|\mathcal{G}'|\in O(|\mathcal{G}|^2)$ berechnet. 
%   Zu jeder \ac{CFG} $\mathcal{G} = (\Sigma, N, P, S)$ gibt es eine äquivalente \ac{CFG} $\mathcal{G}' = (N, \Sigma, P', S)$ ohne Kettenregeln.
\end{lemma}
\begin{proof} Beweisskizze zum Algorithmus in Abb.~\ref{fig:algorithmus-unit}.
  \begin{itemize}
  \item Zur Äquivalenz:
    
    Wir zeigen, dass jeder Schritt, der die \ac{CFG} ändert, eine
    äquivalente \ac{CFG} liefert. 
    Dazu zeigen wir, dass jedes Wort, das in der alten \ac{CFG}
    abgeleitet werden konnte, auch in der neuen \ac{CFG}
    abgeleitet werden kann und umgekehrt. 
    Für Schritt~\ref{alg:unit-step-chain} gilt die
    Äquivalenz sogar für jede Iteration der mittleren for-Schleife. 
    
  \item Zur Kettenregelfreiheit:
    
    In Schritt~\ref{alg:unit-step-DelCycle} wird mindestens ein
    Kettenregelzykel entfernt. Da eine Grammatik nur endlich viele
    Regeln hat, wird nach spätestens $|\mathcal{G}|$ Anwendungen von
    Schritt~\ref{alg:unit-step-DelCycle} der
    Schritt~\ref{alg:unit-step-dag}, in dem alle Zyklen entfernt sind,
    erreicht. 
	
    In der mittleren for-Schleife von
    Schritt~\ref{alg:unit-step-chain} gilt aufgrund der topologischen
    Sortierung immer $i<k$.  Die äußere Schleife hat die Invariante,
    dass am Ende jedes Durchlaufs für $n \ge i \ge j$ keine
    Kettenregel $A_i \to \ldots$ in $P'$ existiert.  Beim Verlassen
    der Schleife ist $j = 1$ und es existieren überhaupt keine
    Kettenregeln mehr.
	
  \item Zu Laufzeit und Größe des Resultates:
	
    Vor Schritt~\ref{alg:unit-step-chain} wird die Grammatik nie
    vergrößert (wichtig sowohl für Laufzeit als auch für Größe!) und
    die Kosten übersteigen $O(|\mathcal{G}|^3)$ nicht.  Die Anzahl der
    Schleifendurchläufe ist jeweils durch $|\mathcal{G}|$ beschränkt.
    Seien $B \to \alpha_1, \ldots, B \to \alpha_n$ alle neuen
    Regeln, die für eine Variable $B\in N$ in
    Schritt~\ref{alg:unit-step-chain} hinzugefügt werden.  Dann gilt,
    dass die Summe der Größe der $\alpha_i$ die Größe der Grammatik
    $|\mathcal{G}|$ nicht übersteigt.  Somit ist die Größe des
    Resultates durch $|\mathcal{G}|^2$ beschränkt.  \qedhere
  \end{itemize}
\end{proof}
...
\begin{description}
 ...
 \item[Ausgabe:] \ac{CFG} $\mathcal{G'} = (\Sigma, N', P', S)$, die
   äquivalent zu $\mathcal{G}$ ist, aber keine Kettenregeln enthält.
\end{description}

  \begin{enumerate}
  \item Setze $N'=N$ und $P' = P$.
  \item Betrachte den gerichteten Graphen $G$, dessen Knotenmenge $N$
    und Kantenmenge $\{(A, B) \mid A \to B \in P' \}$ ist.
  ...
  \item \label{alg:unit-step-dag}
    ...
    Sortiere die Knoten des nun azyklischen Graphen $G$ topologisch\footnotemark als $A_1, \ldots, A_n$, sodass $A_n$ auf keiner linken Seite einer Kettenregel vorkommt.
...
\label{fig:algorithmus-unit}
  \end{figure}
\footnotetext{Topologisches Sortieren ist ein aus der Informatik 2 Vorlesung bekannter Begriff und bezeichnet das Erweitern einer gegebenen partiellen Ordnung zu einer totalen Ordnung.}

\begin{Bsp}
	Sei $\Sigma=\{0,1\}$. Betrachte die \ac{CFG} $\mathcal{G} = (\Sigma, \{S,A,B,C,D,E\}, P, S)$
...
\begin{Satz}[name={[Wortproblem für \acs*{CFL} entscheidbar]}]
	Es gibt einen Algorithmus, der für eine \ac{CFG} $\mathcal{G}$ in \ac{CNF} und ein Wort $w$
...
\begin{proof}[\acsu{CYK}-Algorithmus]
	% Der \ac{CYK}-Algorithmus berechnet bei Eingabe von $\mathcal{G}$ und $w$, ob $w\in L(\mathcal{G})$.
	Der \ac{CYK}-Algorithmus\footnote{Benannt nach Cocke, Younger
          und Kasami, die den Algorithmus unabhängig voneinander
          entdeckt haben. Manchmal auch YCK.} in \autoref{fig:AlgCYK} erfüllt diese Eigenschaften.
	Wir wollten zunächst die Idee des Algorithmus erklären.

        Sei $w = a_1\dots a_n\in\Sigma^*$.
	Definiere für $1\leq i \le j\leq n$:
	\[
	M_{ij} = \{ A \in N \mid A \vdash^*_\mathcal{G} a_i\dots a_j \}\label{eqn:CykMatrixEintragDeriv}\tag{EntryDeriv}
	\]
	...
	Es gilt: $w\in L(\mathcal{G})$ genau dann, wenn $S \vdash^*_\mathcal{G} w$ genau dann, wenn $S \in M_{1n}$.
	\expl{
	...
	Zur Korrektheit: Zeige mittels einer verschachtelten Induktion über Zeilen und Spalten, 
	...
	und jeder Eintrag enthält höchstens $|N|$ Variablen.
	\qedhere
\end{proof}
...
\begin{figure}
	...
	\begin{description}
		...
		\item[Ausgabe:]  $a_1\ldots a_n\in L(\mathcal{G})$?
	\end{description}
...
\begin{Bsp}
	...
	Z.B. in die \ac{CFG} $\mathcal{G'} = (\Sigma, \{S, X, Y, Z, A, B, C\}, P', S)$, deren Regeln $P'$ wie folgt definiert sind.
	\begin{align*}
		S &\-> XY\\
		X &\-> AB \mid AZ\\
		Z &\-> XB\\
...
\begin{Korollar}
	Es gibt einen Algorithmus, der für eine \ac{CFG} $\mathcal{G}$ und ein Wort $w$
	in $O(|w|^3\cdot |\mathcal{G}|^2)$ Schritten und mit $O(|w|^2\cdot |\mathcal{G}|^2)$ Speicherplatz
	entscheidet, ob $w\in L(\mathcal{G})$.
\end{Korollar}

\begin{proof}
	Nach \autoref{satz:CFGtoCNF} kann eine \ac{CFG} in \ac{CNF} 
	konvertiert werden, sodass deren Größe höchstens quadratisch wächst.
	Wende anschließend den CYK Algorithmus auf das Resultat an.
\end{proof}



\begin{Satz}\label{thm:cfl-decidable-emptyness}
...
\begin{Def}
	Sei $\mathcal{G} = (\Sigma, N, P, S)$ eine \ac{CFG}. 
	Ein Nichtterminalsymbol $A\in N$ ist \emph{co-erreichbar}, falls es ein Wort $w\in\Sigma$ existiert, 
...
\begin{proof}
    % Sei $\mathcal{G} = (\Sigma, N, P, S)$ eine \ac{CFG}.
    Die $M$ die Menge der co-erreichbaren Nichtterminale von $\G$ ist gegeben durch
    %
    $$M = \{ A \mid A \vdash^*_\mathcal{G} w, w\in \Sigma^* \}
    \text.$$
    %
    Dann gilt: $L =\varnothing \<=> S\notin M$.
    
    Wir können $M$ wie folgt berechnen:
    \begin{align*}
      M_0 &= \emptyset \\
      M_{i+1} &=  \{ A \mid A \-> \alpha\in P, \alpha\in(\Sigma\cup M_i)^* \}
    \end{align*}
    Es gilt für alle $i\in\N$, $M_i\subseteq M_{i+1} \subseteq M$.
    Da $M\subseteq N$ endlich ist, gibt es ein $n$, sodass $M_n =
    M_{n+1}$ (hier ohne Beweis). Setze $M = M_n$.
    
    Wir können daher $M$ analog zu $\operatorname{Nullable}(\mathcal{G})$ (siehe \autoref{satz:3.nullable}) in $O(|\mathcal{G}|)$ berechnen.
\end{proof}
\begin{Bem}
  Wenn wir in einer Grammatik $\mathcal{G}$ ein nicht co-erreichbares
  Nichtterminalsymbol $A$ und alle Regeln, bei denen $A$ auf der
  linken oder rechten Regelseite auftritt, entfernen, ändert sich
  $L(\mathcal{G})$ nicht.  Wir können eine gegebene Grammatik also
  optimieren, indem wir alle nicht co-erreichbaren
  Nichtterminalsymbole entfernen.
\end{Bem}
...
\begin{proof}[von \autoref{satz:PL für CFL}]\footnote{
...
%   Wir nennen den Knoten 
  Teile $z = uvwxy$ wie in \autoref{zerlegung} skizziert. Es gilt:
      ...
      \item $|vwx| \le 2^{|N|} = n$, da der obere Knoten dessen linke Regelseite mit $A$ beschriftet ist, höchstens $|N|$ Schritte von der Blattebene entfernt und $\mathcal{T}$ ein Binärbaum ist.
      ...
      \end{itemize}
		...
		\begin{subfigure}[b]{.35\linewidth}\centering
				...
				\node[dot,label={above right:$S\rightarrow\alpha$}] (v1) at (0,-1) {};
				\node[dot] (v2) at (-0.25,-1.6) {};
				\node[dot,] (v3) at (0,-2) {};
				\node[dot] (v4) at (-0.5,-2.5) {};
				\node[dot,label={above right:$A\rightarrow \beta$}] (v5) at (0,-3) {};
		...
		\begin{subfigure}[b]{.28\linewidth}\centering
			\begin{tikzpicture}[node distance=.5cm, on grid,
				...
				\node (S) {S\rightarrow\alpha};
				\node[inner sep=2pt] (A) [below=of S.south, xshift=6mm] {A\rightarrow\beta};
		...
		\begin{subfigure}[b]{.29\linewidth}\centering
			\begin{tikzpicture}[node distance=.75cm, on grid,
				...
				\node (0) {S\rightarrow\alpha};
				\node (A1) [below=of 0] {};
				\node () [below=of 0, yshift=-1mm] {\quad A\rightarrow BC};
				\node (A2) [below=of A1] {};
				\node () [below=of A1] {\quad A\rightarrow BC};
				\node (A3) [below=of A2] {};
				\node () [below=of A2] {\quad A\rightarrow BC};
				...
				\node (A4) [below=\Offset of dots] {A\rightarrow\beta};
...
% 		\end{subfigure}
		\caption{Illustration des Argumentes aus dem Beweis von \autoref{satz:PL für CFL}}
		\label{illCfgPl}
...
\begin{proof}
	...
	Sei $n\in\N$ mit $n>0$ die Konstante aus dem \ac{PL}.
	Wir betrachten $z=a^nb^nc^n$. \expl{Es gilt also $|z| = 3n \ge n$.}
	Sei $z = uvwxy$ eine Zerlegung mit $|vx| \ge 1$ und $|vwx| \le n$.
	\expl{Wir zeigen nun, dass $\forall i\in\N: uv^iwx^iy\in L$ nicht gilt (und folgern, dass unsere Annahme falsch war).}
	...
	\end{itemize}
	Die Sprache $L$ ist also nicht kontextfrei\expl{, denn wir erhalten in jedem der fünf Fälle einen Widerspruch zur Aussage des Pumping Lemma}.
\end{proof}
...
\begin{Satz}\label{thm:cfl-not-closed-intersection}
  \ac{CFL} ist \emph{nicht} abgeschlossen unter Durchschnitt ($\cap$).
\end{Satz}
\begin{proof}
 für $n\geq 1$
		...
		Also \ac{CFL} ist nicht abgeschlossen unter Durchschnitt
		(siehe \autoref{bsp:5.planbncn}).
\end{proof}


\begin{Satz}
 	\ac{CFL} ist \emph{nicht} unter Komplement
        ($\overline{\!\phantom{i}\cdot\!\phantom{i}}$) abgeschlossen.
\end{Satz}
\begin{proof}
  Angenommen, CFL wäre abgeschlossen unter Komplement.\\
  ...
  $\Rightarrow \overline{\overline{L}_1\cup \overline{L}_2} = L_1\cap L_2\in\acs*{CFL}$ -- Widerspruch zu \autoref{thm:cfl-not-closed-intersection}. \qedhere 
\end{proof}

\begin{Satz}
  \ac{CFL} ist abgeschlossen unter Schnitt mit der \ac{REG}.
\end{Satz}
\begin{proof}
  Sei $L\in$ CFL, $R$ eine reguläre Sprache, $\G = (\Sigma, N, P, S)$ eine CFG in CNF mit $L(\G) = L$
  und $\A =(\Sigma, Q,\delta,\qinit,F)$ ein \ac{NEA} mit $L(\A) = R$.

  Konstruiere $\G' = (\Sigma, N', P', S')$ wie folgt:
  \begin{itemize}
  \item $N' = Q\times N \times Q \dotcup \{ S' \}$,
  \item $P'$ enthält folgende Regeln
    \begin{itemize}
    \item $S' \to (\qinit, S, q_f)$, für jedes $q_f \in F$;
    \item $(q_0, A, q_2) \to (q_0, B, q_1) (q_1, C, q_2)$, für alle $q_0, q_1, q_2 \in Q$, $A \to BC \in P$;
    \item $(q , A, q') \to a$, für alle $q\in Q$, $q' \in\delta (q, a)$, $A \to a \in P$.
    \end{itemize}
  \end{itemize}
  Jede Ableitung $S' \vdash^*_{\G'} w$ kann durch Löschen der $Q$-Komponenten und
  Überspringen des ersten Ableitungsschritts in eine $\G$-Ableitung
  transformiert werden.
  Ebenso können wir aus der Ableitung einen akzeptierenden Lauf von
  $\A$ auf $w$ extrahieren.
  Also ist $w \in L \cap R$.

  Umgekehrt: Gegeben einen $\G$-Ableitungsbaum $\T$ für $w$ und einen akzeptierenden
  Lauf von $\A$ auf $w$, konstruiere einen $\G'$-Ableitungsbaum
  für $w$ durch eine in-order Traversierung von $\T$. Der Lauf gibt an
  den Produktionen an den Blättern eindeutig den jeweiligen Folgezustand vor.
\end{proof}
