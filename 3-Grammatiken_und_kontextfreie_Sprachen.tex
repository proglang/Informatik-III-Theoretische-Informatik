...
\begin{Bemerkung}
Nach dieser Definition gibt es also insbesondere für jede Regel $\pi$,
bei der nur Terminalsymbole auf der rechten Seite vorkommen, einen
Ableitungsbaum, der die ein-elementige Knotenmenge $\{\pi\}$ und einer
leeren Menge von Kanten hat. 
\end{Bemerkung}
...
Es gibt einen Algorithmus der für eine \ac{CFG} $\mathcal{G}$ in $O(|\mathcal{G}|^3)$ eine äquivalente \ac{CFG} $\mathcal{G}'$ ohne Kettenregeln mit $|\mathcal{G}'|\in O(|\mathcal{G}|^2)$ berechnet.
