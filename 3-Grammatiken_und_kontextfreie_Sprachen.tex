...
Es gilt: $L(\mathcal{G})$ ist die Sprache der Wörter über $\{0,1\}$, die gleich viele Nullen wie Einsen haben:
  ...
  Hierzu definieren wir noch die Hilfsfunktion $d : \Sigma^* \to \Z$:
  \begin{align*}
    d(\Eps) &= 0 \\
    d(1w) &= d(w) + 1 \\
    d(0w) &= d(w) - 1
  \end{align*}
  Per Induktion über $u$ lässt sich leicht zeigen, dass $u \in L \<=>
  ...
  \begin{align*}
    P(n) &= \forall w. |w| = n \=> w \in L \=> w \in L(\mathcal{G})
           \text.
  \end{align*}
  und beweisen sie mit Hilfe des folgenden Beweisprinzips
  (vollständige  Induktion bzw. starke Induktion):
  \begin{gather*}
    \frac{
