...
\begin{proof}
    ...
    Wir definieren:
    Ein Symbol $X \in \Sigma \cup N$ heißt \emph{co-erreichbar}, wenn ein $w \in \Sigma^*$ existiert mit $X \vdash^* w$.
...
\begin{proof}
  ...
  \begin{itemize}
   \item Wir fügen nun eine frische Variable $Y_{\Eps}$ und die Regel $Y_{\Eps}\rightarrow\Eps$ hinzu.
   \item Wir löschen jede Regel der zweiten Form und führen statt dessen die Regel $A \-> aY_{\Eps}$ ein.
  \item wir löschen jede Regel der dritten Form und führen statt dessen die Variable $Y_{a_2}$ und die Regeln $A \-> a_1Y_{a_2}$ und $Y_{a_2} \-> a_2Y_\Eps$ ein.
  \end{itemize}
	Die resultierenden Grammatik $\mathcal{G}=(\Sigma,N',P',S)$ ist äquivalent zu $\mathcal{G}$. (Ohne Beweis)
