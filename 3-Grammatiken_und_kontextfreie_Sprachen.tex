...
Zur Korrektheit:
    ...
    Aufgrund der topologischen Sortierung gilt immer $i<k$ in der zweiten for Schleife.
...
\begin{itemize}
	\item Vor Schritt~\ref{alg:unit-step-chain} wird die Grammatik nie vergrößert und die Kosten übersteigen $O(|\mathcal{G}|^3)$ nicht.
...
\end{description}
  ...
  \item \label{alg:unit-step-dag}
    ...
    Sortiere die Knoten des nun azyklischen Graphen\footnotemark $G$ topologisch als $A_1, \ldots, A_n$, sodass $A_n$ auf keiner linken Seite einer Kettenregel vorkommt.
...
\end{Def}
\begin{Satz}
Es gibt einen Algorithmus der für eine \ac{CFG} $\mathcal{G}$ in $O(|\mathcal{G}|^3)$ eien äquivalente \ac{CFG} $\mathcal{G}'$ in CNF mit $|\mathcal{G}'|\in O(|\mathcal{G}|^2)$ berechnet.
\end{Satz}
