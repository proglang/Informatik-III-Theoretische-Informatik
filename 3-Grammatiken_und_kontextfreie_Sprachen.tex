...
\begin{Satz}[name={[Entscheidbarkeit des Leerheitsproblems für kontextfreie Sprachen]}] % 4.7
  \label{thm:cfl-decidable-emptyness}
    Es gibt einen Algorithmus, der das Leerheitsproblem\footnote{Gefragt ist, ob $L(\mathcal{G}) = \{ w\in\Sigma^* \mid S\overset{*}{\vdash} w \} \overset{?}{=} \emptyset
...
\begin{proof}
    ...
    Da $M\subseteq N$ endlich gibt es ein $n$, sodass $M_n = M_{n+1}$ (hier ohne Beweis).
...
\begin{Bem}
        ...
        Wir können eine gegebenen Grammatik also optimieren indem wir alle nicht co-erreichbares Nichtterminalsymbole entfernen.
\end{Bem}
