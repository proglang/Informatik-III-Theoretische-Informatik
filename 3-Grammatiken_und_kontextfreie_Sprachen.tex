...
\end{figure}
  ...
  \item[IS] $i \rightsquigarrow i+1$
    ...
    \item 
      ...
      werden können:
      \begin{center}
        \tikz[baseline=0cm]{\Tree [.{$A  \vdash_\mathcal{G} A_1 \ldots A_n$} $\T_1$ \edge[draw=none]; {$\dots$} $\T_n$ ]}
      \end{center}
      % \begin{displaymath}
  ...
  Wir zeigen mittels Induktion über den Ableitungsbaum von $A
  \vdash^*_\mathcal{G} \Eps$, dass $\exists i.\, A \in M_i$. 
  \begin{description}
  \item[IA] $A \vdash_\mathcal{G} \Eps$ ist eine Produktion. Also ist
    $A \in M_1$.
  \item[IS] Die Wurzel des Ableitungsbaums ist mit $A \vdash_\mathcal{G} A_1\dots
