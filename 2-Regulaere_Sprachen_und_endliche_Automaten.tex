...
Notation: Sei $w\in\{0,1\}^*$ dann schreiben wir $\operatorname{bin}(w)$ für die Dekodierung der des Bitstings $w$ in eine natürliche Zahl.
Z.B. gilt $\operatorname{bin}(101) = 5$.
...
\vspace{-2em}
\begin{proof}
	Sei $\A=(\Sigma, Q,\delta,q_0,F)$ ein beliebiger \ac{DEA} für $L$.
	...
	\begin{alignat*}{3}
		\text{Nun gilt: }\quad
		\exists u:\; && \tilde\delta(q_0,u)&=q &\qquad& u\text{ Präfix von }z\\
	...
	\end{alignat*}
	\begin{alignat*}{2}
		\text{Es folgt für beliebiges $i\in\N$ :}\quad \tilde\delta(q_0,uv^iw) &= \tilde\delta(q,v^iw)\\
...
\subsection[\acf{NEA}]{Nichtdterministischer endlicher Automat (NEA)}
...
Naiver Lösungsversuch:
	...
	\end{figure}
Problem: Diagramm beschreibt keinen DEA. $q_0$ hat zwei ausgehende Kanten für $1$.
...
\end{Def}
\begin{Satz}[Rabin und Scott]
	Zu jedem \ac{NEA} $\calN$ mit $n$ Zuständen gibt es einen \ac{DEA} $\A$ mit $2^n$ Zuständen, so dass $L(\calN)=L(\A)$.
\end{Satz}
\hide{
\draftnote{4.11.16}
