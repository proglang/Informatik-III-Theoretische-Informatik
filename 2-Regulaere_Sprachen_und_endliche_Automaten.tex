...
Eine Lösung für dieses Problem sahen wir bereits im voherigen Kapitel als wir die (unendlich große) Menge der binärcodierten Primzahlen mit Hilfe der folgenden Zeile darstellten.
\[
L_\mathsf{prim}=\{w\in\{0,1\}^*\mid w \text{ ist Bin\"arcodierung einer Primzahl}\}
\]
Ein weiteres Beispiel ist die folgende Zeile.
\[
L_\mathsf{even}=\{w\in\{0,1\}^*\mid w \text{ die Anzahl der Einsen in $w$ ist gerade }\}
\]
...
Wir nennen dieses Entscheidunsproblem das \emph{Wortproblem}. Eine konkrete Instanz des Wortproblemes wäre z.B. $1100101\in L_1$? oder $1100101\in L_2$?

Die obige Darstellungen der unendlichen Mengen $L_1$ und $L_2$ sind zwar sehr kompakt, 
wir können daraus aber nicht direkt ein Vorgehen zur Lösung des Wortproblemes ableiten.
Wir müssen zunächst verstehen was die Begriffe ``Binärcodierung'', ``Primzahl'' oder ``gerade Anzahl'' bedeutet und für $L_1$ und $L_2$ jeweils einen Algorithmus zur Entscheidung entwickeln.
...
\begin{Def}[name={[Die durch einen \acs*{DEA} erkannte Sprache]}]\label{def:2.sprache}
        Sei $\A=(\Sigma, Q,\delta,\qinit,F)$. Die \emph{von $M$ erkannte Sprache} ist
...
$$L_\mathsf{even}=\{w\in\{0,1\}^*\mid w \text{ die Anzahl der Einsen in $w$ ist gerade }\}$$ 
aus der Einleitung dieses Kapitels hat die folgende graphische Representation.
%         $L=\{w\in\{0,1\}^* \mid w \text{ enthält gerade Anzahl von 0 und gerade Anzahl von 1}\}$
  \begin{center}
                \begin{tikzpicture}[circle/.style={
                        shape=circle,
...
Frage: Gegeben zwei reguläre Sprachen $L_1$, $L_2$ über einem gemeinsamen Alphabet $\Sigma$ ist dann auch die Vereinigung $L_1\cup L_2$ eine reguläre Sprache? Wir beantworten diese Frage mit dem folgenden Satz.
...
\datenote{25.10.17}
Sei $A_1$ der folgende \ac{DEA} über dem Alphabet $\Sigma=\{0,1\}$ dessen graphische Repräsentation nahezu mit $A_{\mathtt{Maccaron}}$ identisch ist.
...
\end{tikzpicture}
\end{center}
Der Produktautomat für Schnitt von $A_1$ und $A_\mathsf{even}$ hat die folgende graphische Repräsentation.
...
\begin{Def}[name={[Äquivalenz von \acs*{DFA}-Zuständen]}] %\rlnote{Def.-Num. überprüfen}
  Wir nenen zwei Zustände $q,p\in Q$ eines \ac{DFA} \emph{äquivalent}, geschrieben $p\equiv q$, falls
  \begin{displaymath}
...
Die Zustände $q_{11}$, $q_{20}$ und $q_{21}$ aus \autoref{bsp:2.schnitt} sind äquivalent.
