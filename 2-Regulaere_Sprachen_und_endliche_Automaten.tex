...
\begin{Bsp}~
\begin{enumerate}
 \item \texttt{(I+i)nfo(3+III)} ist ein regulärer Ausdruck über dem Alphabet der ASCII Zeichen.
 \item $(\0\1)^*+a$ ist ein regulärer Ausdruck über $\Sigma=\{a\}$.
 \item $\0\1\0\1$ ist über jedem Alphabet ein regulärer Ausdruck.
...
Im verbleibenden Teil des Abschnittes zu regulären Ausdrücken werden wir Zeigen, dass wir für jeden DEA einen regulären Ausdruck mit der gleichen Sprache konstruieren können.
Wir beschreiben zunächst ein mögliches Vorgehen an einem Beispiel
\begin{Bsp} Betrachten den aus \autoref{bsp:2.mod3} (Pumping Lemma) bekannten DEA.
\begin{center}
    \begin{tikzpicture}[>=stealth, shorten >=1pt, on grid, node distance=2cm, initial text=]
        \node[state, initial, accepting] (q0) {$q_0$};
        \node[state] (q1) [right=of q0] {$q_1$};
...
Zum Beweis (bzw.\ zur Konstruktion) der Richtung "`\=>"' verwenden wir eine Rechenregel zum Lösen von Gleichungen über Sprachen.
