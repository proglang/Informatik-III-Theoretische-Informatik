\section[Reguläre Sprachen und endliche Automaten]{Reguläre Sprachen und endliche Automaten}\label{sec:reglang}\datenote{22.04.26}
Wie können wir potentiell unendlich große Mengen von Wörtern darstellen?
Eine Lösung für dieses Problem sahen wir bereits im vorherigen Kapitel, als wir die (unendlich große) Menge der binär codierten Primzahlen mit Hilfe der folgenden Sprache darstellten.
\[
...
Wir nennen dieses Entscheidungsproblem das \emph{Wortproblem}. Eine konkrete Instanz des Wortproblems wäre z.B.\ "`$1100101\in L_\mathsf{prim}$?"' oder "`$1100101\in L_\mathsf{even}$?"'
...
\item[Lesekopf] ~\\
  ...
  \end{itemize}
\item[Zustände] Zu jedem Zeitpunkt befindet sich der Automat in einem Zustand $q$ aus einer \emph{endlichen} Zustandsmenge $Q$.
\item[Startzustand] $\qinit \in Q$
\item[Akzeptierende Zustände] $F \subseteq Q$ 
\item[Transitionsfunktion] Im Zustand $q$ beim Lesen von $a$ gehe in Zustand $q' := \delta(q,a)$.
\end{description}
...
% \end{Bsp}
\begin{Def}[\acs*{DEA}]
        Ein \emph{\acf{DEA}}\footnote{engl.\ \acsu{DFA} $\hat=$ \acl{DFA}} ist ein 5-Tupel
...
Hierbei sei $\A= (\Sigma, Q,\delta,\qinit,F)$ ein \acs*{DEA}.
% \begin{Bsp}
...
% \end{Bsp}
% \begin{Def}
%   Definiere eine Relation ${}\Rightarrow{} = {}\Rightarrow_\delta$ auf $Q \times \Sigma^*$ durch
%   $(q, aw) \Rightarrow (\delta (q,a), w)$. Dazu sei $\Rightarrow^*$ die reflexive, transitive Hülle.
% \end{Def}
\begin{Def}[Induktive Erweiterung von $\delta$ auf Wörter]\label{def:2.deltaschlange}
  ...
  \end{enumerate}
\end{Def}
\begin{Def}[Durch einen \acs*{DEA} akzeptierte Sprache]\label{def:2.sprache}
        Sei $\A=(\Sigma, Q,\delta,\qinit,F)$. 
        Ein Wort $w\in\Sigma^*$ wird von $\A$ \emph{akzeptiert}, falls $\tilde\delta(\qinit,w)\in F$.
%        Ein Wort $w\in\Sigma^*$ wird von $\A$ \emph{akzeptiert}, falls $(\qinit,w) \Rightarrow^* (q, \Eps)$ und $q\in F$.
...
%        \[ L(\A) = \{ w\in\Sigma^* \mid (\qinit,w) \Rightarrow^* (q, \Eps), q\in F \}. \]
...
Frage: Angenommen wir haben zwei \ac{DEA}s $A_1$ und $A_2$, können wir dann immer einen \ac{DEA} konstruieren der den Durchschnitt $L(A_1)\cap L(A_2)$ akzeptiert?
...
\begin{Def}[Produktautomat für Durchschnitt]\label{def:2.Produktautomat}
Seien $A_1=(\Sigma, Q_1,\delta_1,\qinit_1,F_1)$ und $A_2=(\Sigma, Q_2,\delta_2,\qinit_2,F_2)$ \ac{DEA}s.
Wir definieren den \emph{Produktautomaten für Durchschnitt} als den \ac{DEA} $A_\cap=(\Sigma, Q_\cap,\delta_\cap,\qinit_\cap,F_\cap)$ mit den folgenden Komponenten.
		\begin{align*}
			Q_\cap &= Q_1\x Q_2\\
...
Der Produktautomat für den Durchschnitt von $A_1$ und $A_\mathsf{even}$ hat die folgende graphische Repräsentation, wobei wir um Platz zu sparen "`$q_{ij}$"' statt "`$(q_i, q_j)$"' schreiben.\datenote{28.04.26}
...
\begin{Satz}\label{satz:2.Produktautomat}
  Für beliebige \ac{DEA}s $A_1$ und $A_2$ akzeptiert der entsprechende Produktautomat für Durchschnitt die Sprache $L(A_1)\cap L(A_2)$.
\end{Satz}

\input{proofs/regular-intersection-proof}
...
% $A_\cap=(\Sigma, Q_\cap,\delta_\cap,\qinit_\cap,F_\cap)$ der zugehörige Produktautomat für den Durchschnitt.
% Wir zeigen zunächst per Induktion über $w$, dass für alle $w\in\Sigma^*$, für alle $q_1\in Q_1$ und für alle $q_2\in Q_2$ die folgende Gleichung $\varphi(w)$ gilt.
% $$\varphi(w) := \tilde{\delta}_\cap((q_1,q_2),w) = (\tilde{\delta}_1(q_1,w), \tilde{\delta}_2(q_2,w))$$
% Der Induktionsanfang $\varphi(\Eps)$ folgt dabei direkt aus \autoref{def:2.deltaschlange}.
...
Beobachtung: Der Zustand $q_{01}$ im Beispiel~\ref{bsp:2.schnitt}
scheint nutzlos. Wir charakterisieren die Nützlichkeit eines Zustands
in einem DEA $\A = (Q, \Sigma, \delta, \qinit, F)$ formal wie folgt.

\begin{Def}
  Ein Zustand $q \in Q$ heißt \emph{erreichbar}, falls ein $w \in \Sigma^*$ existiert, sodass $\tilde \delta(\qinit, w) = q$.
...
Bemerkung: Die Menge der erreichbaren Zustände kann mit dem folgenden Verfahren in $O(|Q|\cdot|\Sigma|)$ berechnet werden.
  ...
  \item Wende Tiefensuche ab $\qinit$ an und markiere dabei alle besuchten Zustände.
  \item Genau die markierten Zustände sind erreichbar.
  \end{itemize}
...
\begin{Def}[name={[Äquivalenz von \acs*{DEA}-Zuständen]}] %\rlnote{Def.-Num. überprüfen}
  Zwei Zustände $p,q\in Q$ eines \ac{DEA} sind \emph{äquivalent}, geschrieben $p\equiv q$, falls
  \begin{displaymath}
...
Hiermit endet der Exkurs zu Äquivalenzrelationen; wir wollen mit
diesem Wissen die oben definierte Relation $\equiv\; \subseteq Q\times
Q$ genauer analysieren. 
\end{comment}
\begin{lemma}[name={[$\equiv$ ist Äquivalenzrelation]}]
        Die Relation "`$\equiv$"' ist eine \emph{Äquivalenzrelation}.
\end{lemma}
\begin{proof}
  Zu zeigen ist, dass $\equiv$ reflexiv, transitiv und symmetrisch
  ist.
  \begin{itemize}
  \item Die Relation $\equiv$ ist offensichtlich reflexiv.
  \item Die Symmetrie und Transitivität von~$\equiv$ folgen aus der
    Symmetrie und Transitivität der logischen Interpretation von
    "`genau dann, wenn"' (gdw).
  \end{itemize}
\end{proof}
...
\begin{Def}[name={[Äquivalenzklassenautomat]}]
        ...
        \begin{align*}
          ...
          \delta_\equiv(q_\equiv,a) &= [\delta(q,a)] & \text{ wobei }q\in q_\equiv \text{ beliebig gewählt } & (\text{also }q_\equiv = [q])\\
          \qinit_\equiv &= [\qinit] \\
...
\end{Def}
\begin{Bsp}
Der Äquivalenzklassenautomat $\A_\equiv$ zum \ac{DEA} aus \autoref{bsp:2.schnitt} hat das folgende Zustandsdiagramm.\datenote{29.04.26}
 % \eqref{bsp:3.1}:
\begin{center}
   \captionsetup{type=figure}
        \begin{tikzpicture}
...
\begin{proof}\ 
        \begin{enumerate}
                ...
                \item $L(\A)=L(\A_\equiv)$:
                Zunächst zeigen wir per Induktion über $w$, dass für alle $w\in\Sigma^*$ und alle $q\in Q$ die folgende Äquivalenz gilt.
                \[\varphi(w) := \quad \tilde\delta(q,w)\in F \<=> \tilde\delta_\equiv([q],w)\in F_\equiv\]
                
                
                I.A. $(\varphi(\Eps))$: $\tilde\delta(q,\Eps)=q\in F \<=> \tilde\delta_\equiv([q],\Eps)=[q]\in F_\equiv$\\
                I.S. ($\forall a,w: \varphi(w)\Rightarrow \varphi( aw)$): \begin{align*}
                ...
                \end{align*}
                Damit zeigen wir nun $L(\A)=L(\A_\equiv)$. Sei $w\in\Sigma^*$.
                \begin{align*}
                 w\in L(\A)
...
Wir werden später (\-> Satz von Myhill-Nerode) sehen, dass $\A_\equiv$
ein \acs*{DEA} ist, der $L(\A)$ akzeptiert, sodass kein \acs*{DEA},
der ebenfalls $L(\A)$ akzeptiert, weniger Zustände haben kann.
...
Angenommen, wir haben für eine gegebene Sprache $L$ einen ersten
Entwurf für einen \ac{DEA} $\A$ konstruiert.
In diesem führen sowohl das Wort $u\in\Sigma^*$
...
\end{figure}
\begin{itemize}
 \item Angenommen, es gibt ein Wort $w\in\Sigma^*$, dass die Äquivalenz $uw\in L \Leftrightarrow vw\in L$ verletzt.
 Dann muss der Entwurf noch einen Fehler enthalten und wir müssen dafür sorgen, 
 dass $\qinit$ nicht sowohl von $u$ als auch von $v$ in $q_{42}$ überführt wird.
 \item Angenommen, es gilt für alle Wörter $w\in\Sigma^*$, dass $uw\in L\Leftrightarrow vw\in L$.
 Dann wissen wir, dass es eine gute Idee war, $\qinit$ mit $u$ und $v$ in den gleichen Zustand zu führen.
\end{itemize}
...
\begin{lemma}\label{lem:2.nerode-rechtskongruent}
   Die Nerode-Relation ist rechtskongruent.
\end{lemma}
Dafür starten wir mit folgender Beobachtung.
\begin{lemma}\label{lem:2.nerode-rechtsabgeschlossen}
  $\forall u, v \in \Sigma^*. \forall a \in \Sigma$. $(u,v) \in R_L \=> (ua, va) \in R_L$
\end{lemma}
\begin{proof}
     \begin{eqnarray*}
       &&(u,v) \in R_L\\
       &\Leftrightarrow& \forall w\in\Sigma^*: uw\in L \<=> vw\in L \\
       && \text{wähle alle $w$ der Form $w = aw'$} \\
       &\Rightarrow& \forall a\in\Sigma:\forall w'\in\Sigma^*: uaw'\in L \<=>
                     vaw'\in L \\
       & \<=> & (ua, va) \in R_L
     \end{eqnarray*}
     D.h. $(u,v) \in R_L \Rightarrow \forall a\in\Sigma: (ua, va) \in
     R_L$.
\end{proof}
\begin{proof}[\autoref{lem:2.nerode-rechtskongruent}]
     
     Wir wählen die folgende Induktionsbehauptung
     \begin{displaymath}
       \forall w\in\Sigma^*: \forall
       (u,v) \in R_L : (uw,vw)\in R_L
     \end{displaymath}
     und führen eine Induktion über $w$ durch.

     I.A.  ($w=\Eps$): $ (u\Eps,v\Eps)=(u,v)\in R_L$ \\
     I.S.  ($w\rightsquigarrow a.w$): Zeige nun $\forall
     (u,v) \in R_L : (uaw,vaw)\in R_L$.\\
     Sei $(u,v) \in R_L$, dann ist nach \autoref{lem:2.nerode-rechtsabgeschlossen} auch $(ua, va)  \in R_L$.
     Daraus liefert die Induktionsbehauptung sofort $(uaw, vaw)  \in R_L$.

%                 Sei $(u,v)\in R_L$. Zeige $\forall w\in\Sigma^*: (uw,vw)\in R_L$.
%                 Wir führen diesen Beweis per Induktion über $w$.\\
%                 I.A. ($w=\Eps$) Es ist $ (u\Eps,v\Eps)=(u,v)\in R_L$.\\
% %                 I.S. Sei Aussage gezeigt für alle Wörter $w'$ mit $|w'| \le k$.\\
%                 I.S. ($w\rightsquigarrow a.w$) Betrachte mit $w=w'a$ ein beliebiges Wort der Länge $n+1$.
...
\end{Bsp}
\begin{Bsp}
      Für ein beliebiges Alphabet $\Sigma$ gilt:\datenote{05.05.2026}
      \begin{enumerate}
...
\begin{Bsp}\label{bsp:2.lettercountNerode}
...
\begin{Satz}[Myhill und Nerode] % 3.4
        Für Sprachen $L\subseteq \Sigma^*$ sind folgende Aussagen äquivalent.
        \begin{enumerate}
...
\begin{proof}
  ...
  \end{center}
        ...
        \paragraph{(1) \=> (2)} Sei $\A$ ein \ac{DEA}. Es gilt
  \begin{displaymath}
    L(\A)=\{w \mid \tilde\delta(\qinit,w)\in F \} = \bigcup_{q \in F} \{ w \mid \tilde\delta(\qinit,w) = q\}.
\end{displaymath}
%
Nun sind die Mengen $\{ w \mid \tilde\delta(\qinit,w)=q \}$ für erreichbare $q\in Q$ genau die Äquivalenzklassen der Relation $R_\A$ aus \autoref{Bsp:R_m}, einer rechtskongruenten Äquivalenzrelation. 
Der Index von $R_\A$ ist die Anzahl der \emph{erreichbaren} Zustände und somit endlich: $\operatorname{\operatorname{Index}}(R_\A) \le |Q| < \infty$.
        
\paragraph{(2) \=> (3)} Sei $R$ eine rechtskongruente
Äquivalenzrelation mit endlichem Index, sodass $L$ die Vereinigung von
$R$-Äquivalenzklassen ist. 
...
\paragraph{(3) \=> (1)} Gegeben $R_L$, konstruiere $\A=(Q,\Sigma,\delta,\qinit,F)$
    ...
    \item $\delta(q,a) = [wa]$ \quad für ein $w\in q$---wohldefiniert (vgl.\ \autoref{lem:2.nerode-rechtsabgeschlossen})
    \item $\qinit = [\Eps]$
    \item $F = \{ [w] \mid w\in L \}$
    \end{itemize}
    Wir wollen nun $L(\A)=L$ zeigen. Dafür beweisen wir zunächst per Induktion über $w$ die folgende Eigenschaft.
        \begin{displaymath}
      \forall w\in\Sigma^* :
    ...
    I.S. ($w\rightsquigarrow a.w$)
      \begin{displaymath}
        \begin{array}{lcl}
        \tilde\delta([v],a.w) &=& \tilde\delta(\delta([v],a),w) \\
...
\begin{eqnarray*}
        ...
        & \text{gdw} & [w]\in F, \quad \text{(per Induktion gezeigte Eigenschaft für $v=\Eps$)}\\
        & \text{gdw} & w\in L
        \qedherefixeqnarray
\end{eqnarray*}
\end{proof}
...
\end{Korollar}
\begin{proof}
        Sei $\A'= (Q', \dots)$ ein \emph{beliebiger} \ac{DEA} mit $L(\A')=L$.
        
        Aus "`\ref{itm:Nerode1} {\=>} \ref{itm:Nerode2}"' wissen wir, dass $\operatorname{Index}(R_{\A'}) \le |Q'|$ gilt.
        ...
        Aus "`\ref{itm:Nerode3} \=> \ref{itm:Nerode1}"' erhalten wir $\A= (Q, \dots)$ mit $|Q|=\operatorname{Index}(R_L)$.
        
        Wegen $|Q|=\operatorname{Index}(R_L) \leq \operatorname{Index}(R_{\A'})\le |Q'|$  hat $\A$ nicht mehr Zustände als $\A'$.
\end{proof}




\subsection[Nichtdeterministischer endlicher Automat (NEA)]{\Acf{NEA}}
\datenote{12.05.2026}
Aufgabe: Konstruiere für eine natürliche Zahl $n\in\N$ einen \acs*{DEA} für die folgende Sprache.
	\begin{align*}
		L_n &= \{ w\in\{0,1\}^* \mid |w|\ge n\text{ und das $n$-letzte Symbol von $w$ ist 1} \}
	\end{align*}
...
\begin{Def}[\acs*{NEA}]
        Ein \emph{\acf{NEA}}\footnote{engl.\  \acsu{NFA} $\hat=$ \acl{NFA}} ist ein 5-Tupel
...
Dabei ist
        ...
        \end{itemize}
\end{Def}
Bemerkung: Die Definition des \acs*{NEA} unterscheidet sich vom
\acs*{DEA} nur in der Transitionsfunktion. Beim \acs*{DEA} ist der
Bildbereich der Transitionsfunktion die Menge der Zustände $Q$. Beim
\acs*{NEA} ist der Bildbereich die Potenzmenge $\mathcal{P}(Q)$ der
Zustandsmenge~$Q$. 
Analog zu \acs*{DEA}s werden wir auch \acs*{NEA}s mit Hilfe eines Zustandsdiagramms beschrieben. So beschreibt \autoref{fig:2.nletztesNea} für jedes $n\in\N$ einen \acs*{NEA} für die Sprache $L_n$.

Im Folgenden sei $\calN$ immer ein \acs*{NEA}.
\begin{Def}[Lauf eines Automaten]
  Die Funktion $\Run : Q \x \Sigma^* \to \mathcal{P}(Q^*)$ ist so definiert, dass $\Run (q, w)$ die Folgen von Zuständen liefert, die $\calN$ vom Zustand $q$ angesetzt auf die Eingabe $w$ durchläuft. 
  \begin{align}
    \Run(q, \Eps) & = \{q\} \\
    \Run(q, a.w)  & = \{ q. \overline{q} \mid \overline{q} \in \Run (q', w), q' \in \delta (q, a) \}
  \end{align}
  Ein Lauf $q_0\dots q_n \in \Run (q_0, w)$  heißt \emph{initial}, falls $q_0 = \qinit$.
  Er heißt \emph{akzeptierend}, falls $q_n\in F$.
\end{Def}
\begin{Def}[Akzeptanz]
	...
	$$L(\calN)=\{ w\in\Sigma^* \mid \exists\text{ initialer, akzeptierender Lauf von $\calN$ über }w \} \text.$$
\end{Def}
...
\begin{proof}[von \autoref{satz:2.rabinscott}]
        ...
        Dafür beweisen wir zunächst per Induktion über  $w$ die folgende Eigenschaft:
        
        $\forall w\in\Sigma^*: \forall p\in Q_\calP\backslash\{\{\}\}: \forall q\in Q:$
        $$q\in\tilde\delta_\calP(p,w) \Leftrightarrow \exists q_0 \in p. {q_0, q_1,\ldots, q_n} \in \Run (q_0, w), \text{ sodass } q_n=q$$
        I.A. $(w=\Eps)$: $q\in\tilde\delta_\calP(p,\Eps) = p$ wähle
        $q_0 = q$.\\
        ...
        \begin{eqnarray*}
                ...
                & \stackrel{\text{I.V.}}{\Leftrightarrow} & \hspace*{-2.5mm} \exists q_1 \in \delta_\calP(p,a). \  {q_1,q_2,\ldots, q_n}\in \Run(q_1,w'), \text{ sodass } q_n=q\\
                & {\Leftrightarrow} & \hspace*{-2.5mm} \exists q_0 \in p. \  q_1 \in \delta_\calP(p,a). \  {q_1,q_2,\ldots, q_n}\in \Run(q_1,w'), \text{ sodass } q_n=q\\
                & {\Leftrightarrow} & \hspace*{-2.5mm} \exists q_0 \in p. \   {q_0,q_1,q_2,\ldots, q_n}\in \Run(q_1,aw'), \text{ sodass } q_{n+1}=q\\
        \end{eqnarray*}
        Mit Hilfe dieser Eigenschaft zeigen wir nun die Gleichheit $L(\A_\calP)=L(\calN)$.
        \begin{align*}
         w\in L(\A_\calP) 
         &\<=> \tilde\delta_\calP(\qinit_\calP,w)\in F_\calP\\
         &\<=> \exists p_f\in F_\calP : \tilde\delta_\calP(\qinit_\calP,w) = p_f\\
         &\<=> \exists q_f\in F : q_f \in \tilde\delta_\calP(\qinit_\calP,w) \\
         &\<=> \exists q_0 \in \qinit_\calP = \{ \qinit_\A \}. \  {q_0, q_1,\ldots, q_n} \in \Run(q_0, w), \text{ sodass } q_n = q_f \in F\\
...
\begin{Bsp}\label{bsp:2.centeredPL}%\datenote{9.11.18}
...
\subsection{\texorpdfstring{$\Eps$}{epsilon}-Transitionen}\label{sec:2.EpsNea}
...
Wir nennen solch einen Zustandsübergang $\Eps$-Transition und definieren einen Automaten, der solche Zustandsübergänge haben kann, wie folgt.
\datenote{19.05.2026}

\begin{Def}[$\Eps$-NEA]\label{def2.EpsNea}
        Ein \emph{nichtdeterministischer endlicher Automat mit $\Eps$-Transitionen} ist ein 5-Tupel
...
Bemerkung: Wenn $f : A \to \calP (B)$ ist, dann schreiben wir $\hat{f} : \calP (A) \to \calP (B)$ für die Fortsetzung von $f$ auf Mengen. D.h. $\hat{f} (M) = \bigcup_{a \in M} f(a)$, falls $M \subseteq A$.
Z.B. verwenden wir $\hat\delta : \calP (Q) \times (\Sigma \cup\{\Eps\}) \to \calP (Q)$.

Bemerkung: Funktionen $f, g : A \to \calP (B)$ können bzgl. ihres Bildbereichs geordnet werden. Wir schreiben $f \subseteq g$, falls $\forall a \in A. f(a) \subseteq g(a)$.
Diese Relation auf $A \to \calP (B)$ ist eine partielle Ordnung mit kleinstem Element $f_0 : a \mapsto \emptyset$, die Funktion, die jedes $a\in A$ auf die leere Menge abbildet.
...
Der $\Eps$-Abschluss ist eine Abbildung, die jedem Zustand $q$ die Menge der Zustände zuordnet, die von $q$ über $\Eps$-Transitionen erreichbar sind.
...
\begin{Def}
 Der \emph{$\Eps$-Abschluss} $\ecl_\B:Q\rightarrow\calP(Q)$ ist die kleinste Abbildung (bzgl. $\subseteq$), die für alle $q,q',q''\in Q$ die folgenden Eigenschaften erfüllt:
 \begin{align*}
   & q\in\ecl_\B(q)
   &
   & \frac{q'\in\ecl_\B(q) \qquad q''\in\delta(q',\Eps)}{q''\in\ecl_\B(q)}
 \end{align*}
 % \begin{eqnarray*}
 %  & q\in\ecl_\B(q) \\
 %  & q'\in\ecl_\B(q) \text{ und } q''\in\delta(q',\Eps) \quad\Rightarrow\quad 
 %  \qedherefixeqnarray
 % \end{eqnarray*}
\end{Def}
...
% Als Nächstes definieren wir eine dreistellige Relation, die uns für je
% zwei Zustände sagt, welche Wörter den Automaten vom ersten Zustand in
% den zweiten Zustand überführen. Der Name der Relation "`$\reach$"'
% soll dabei an des englische Wort "`reachability"' erinnern.  

Als Nächstes definieren wir eine mengenwertige Funktion, die uns für
einen Zustand $q$ und ein Wort $w$ sagt, welche Zustände von $q$ durch
$w$ erreichtbar sind. Der Name der Funktion "`$\reach$"'
soll dabei an des englische Wort "`reachability"' erinnern.  

\begin{Def}
  % Die \emph{Erreichbarkeitsfunktion} $\reach_\B : Q\times\Sigma^*\to \calP(Q)$ ist die kleinste Relation, die für alle $q,q',q'',q'''\in Q$ und für alle $w\in\Sigma^*$ die folgenden Eigenschaften erfüllt:
  % \begin{align*}
  %   & \frac{q'\in\ecl_\B(q)}{(q,\Eps,q')\in\reach_\B}
  %   &
  %   & \frac{q'\in\ecl_\B(q) \qquad q''\in\delta(q',a) \qquad (q'',w,q''')\in\reach_\B}{(q,aw,q''')\in\reach_\B}
  % \end{align*}
  Die \emph{Erreichbarkeitsfunktion} $\reach_\B : Q\times\Sigma^*\to \calP(Q)$ ist definiert durch:
  \begin{align*}
    \reach_\B (q, \Eps) & = \ecl_\B(q) \\
    \reach_\B (q, a.w)  & = \bigcup\{ \reach_\B (q'', w) \mid  q' \in \ecl_\B (q), q'' \in \delta (q', a) \} \\
                        & = \bigcup\{ \reach_\B (q'', w) \mid  q'' \in \hat\delta (\ecl_\B (q), a) \} \\
    & = \widehat{\reach_\B} (\hat\delta (\ecl_\B (q), a), w)
  \end{align*}
%   \begin{eqnarray*}
...
% \end{Def}
\begin{Def}\label{def:2.EpsNeaSprache}
 Ein Wort $w\in\Sigma^*$ wird von $\B$ \emph{akzeptiert}, wenn $\reach_\B(\qinit,w) \cap F \ne \emptyset$.
 Die von $\B$ akzeptierte Sprache $L(\B)$ ist die Menge der von $\B$ akzeptierten Wörter, d.h.\
	$L(\B)=\{ w\in\Sigma^* \mid \reach_\B(\qinit,w) \cap F \ne \emptyset \}$.
\end{Def}

Offensichtlich gibt es zu jedem \ac{NEA} $\calN$ einen $\Eps$-NEA $\B$, der die gleiche Sprache akzeptiert.
Die Konstruktion ist einfach: Erweitere die Transitionsfunktion um $\delta(q,\Eps)=\{\}$ für alle $q\in Q$.
Für die Sprachgleichheit zeigen wir mittels Induktion über $w$, dass für alle Wörter $w \in \Sigma^*$ gilt:
$$\exists \text{ Lauf } q_0,q_1,\ldots, q_n \text{ von $\calN$ über } w \quad\Leftrightarrow\quad q_n \in \reach_\B(q_0,w)$$
...
\begin{Satz}\label{satz:2.EpsElim}%\datenote{14.11.18}
...
\begin{Def}[$\Eps$-freier Automat]
 Für einen gegebenen $\Eps$-NEA $\B= (\Sigma, Q,\delta,\qinit,F)$ definieren wir den \acs*{NEA} $\calN= (\Sigma, Q,\delta_\calN,\qinit,F_\calN)$ mit
        ...
        $$\delta_\calN(q,a)=\bigcup\{\ecl_\B(q'') \mid  q''\in\hat\delta(\ecl_\B(q),a)\}$$
        $$F_\calN=\{q\in Q\mid \ecl_\B(q) \cap F \ne \emptyset \}$$
...
\begin{proof}[von \autoref{satz:2.EpsElim}: $\Eps$-Eliminierung]
        Zeige $L(\calN)=L(\B)$. Dabei verwenden wir die folgende Eigenschaft, die wir mittels Induktion über die Länge von $w$ in den Übungen zeigen werden.
        
        $\forall w\in\Sigma^+\; \forall q, q'\in Q:$
        $$q'\in\reach_\B(q,w) \Leftrightarrow \exists \underbrace{q_0, q_1,\ldots, q_n}_{\text{Lauf}}\in Q, \text{ sodass }  n=|w|, q_0=q \text{ und } q_n=q'$$
...
%         \end{eqnarray*}
        ...
        \begin{eqnarray*}
         w\in L(\B) \hspace*{-1mm}
         &\stackrel{\text{\autoref{def:2.sprache}}}{\<=>} &\reach_\B (\qinit, w)\cap F \ne\emptyset\\
        ...
        \end{eqnarray*}
        Der Fall $w = \varepsilon$ folgt mit $\varepsilon \in L(\B) \Leftrightarrow  F \cap \ecl_\B(\qinit) \ne \emptyset \Leftrightarrow \qinit \in F_\calN \Leftrightarrow \varepsilon \in L(\calN)$.
\end{proof}
...
\begin{Def}
Gegeben zwei NEA-$\Eps$ $\B_i= (\Sigma, Q_i,\delta_i,\qinit_i,F_i)$, $i = 1,2$, definieren wir den \emph{$\Eps$-NEA für Konkatenation} $\B= (\Sigma, Q,\delta,\qinit,F)$ wie folgt.
                \begin{align*}
                ...
                \delta(q,a) &=
                  ...
                  \delta (q, \Eps) &=
                                     \begin{cases}
                                       \delta_1 (q,\Eps) \cup \{\qinit_2\} & q \in F_1 \\
                                       \delta_1 (q, \Eps) & q \in Q_1 \setminus F_1 \\
                                       \delta_2 (q, \Eps) & q \in Q_2
                                     \end{cases} \\
                \qinit & = \qinit_1\\
                F &= F_2 \qedhere
                \end{align*}
...
\end{lemma}
\begin{proof}
 Zeige mittels Induktion über $w_1$, dass $\forall w_1,w_2\in\Sigma^*, \forall q_1\in Q_1, \linebreak[1] \forall q_1'\in F, \forall q_2'\in Q_2$ die folgende Eigenschaft gilt.
 \[
   q_1' \in \reach_{\B_1}(q_1,w_1) \text{ und } q_2' \in
   \reach_{\B_2}(\qinit,w_2) \quad\Leftrightarrow\quad q_2' \in \reach_\B(q_1,w_1w_2)
 \]
...
\begin{itemize}
 ...
 \item vom neuen Startzustand einen $\Eps$-Transition zum alten Startzustand hinzufügen und
 \item von jedem akzeptierenden Zustand einen $\Eps$-Transition zum alten Startzustand hinzufügen.
\end{itemize}
\[
                \delta_*(q,x) =
                        \begin{cases}
...
\begin{tabular}{cl@{\quad}l}
 $\cap$ & (Durchschnitt) & \autoref{satz:2.Produktautomat}\\
...
\subsection{Reguläre Ausdrücke}%\datenote{16.11.18}
...
\begin{Def}
  Sei $\A=(\Sigma,Q,\delta,\qinit,F)$ ein \ac{DEA}.
...
\begin{Bsp}\label{bsp:2.mod3gleichungen}%\datenote{21.11.18}
...
\begin{proof}[Teil 1]
  Durch Einsetzen in die Gleichung rechnen wir nach, dass $A^*B$ tatsächlich eine Lösung ist. 
  $$ \underbrace{A^*B}_X = (AA^*\cup \{\Eps\})B = A\cdot (\underbrace{A^*B}_X)\cup B
  \text.$$
  Daraus folgt $A^*B \subseteq X$.
\end{proof}

\input{proofs/arden-proof.tex}
...
%  Sei $w \in X \setminus A^*B$ solch ein Wort minimaler Länge.

%  Wegen $X = AX \cup B$ gilt:
%  \begin{align*}
%    X &= AX \cup B \\
%      &=  A ( AX \cup B) \cup B &
%                                & = A^2X \cup AB \cup B \\
%      &= A^2 ( AX \cup B) \cup AB \cup B && = A^3X \cup A^2B \cup AB
%                                            \cup B\\
%      &= \dots &&= A^nX \cup A^{n-1}B \cup \dots \cup AB \cup B
%  \end{align*}
%  Daher muss es ein $n>0$ geben, sodass  $\underbrace{A^nX}_{\ni w}
%  \cup \underbrace{A^{n-1}B \cup \dots \cup AB \cup B}_{\not\ni w}$
%  ist. Also hat $w$ die Form
%  $w = u_1\dots u_n w'\text{ mit } u_1,\dots,u_n\in A, \forall i: u_i
%  \ne \Eps \text{ und } w'\in X$.
%  Fallunterscheidung:
%  \begin{itemize}
%   \item $w' \in A^*B$ \ $\Rightarrow w\in A^nA^*B\subseteq A^*B$ -- Widerspruch!
%   \item $w' \in X \setminus A^*B$ \ $\Rightarrow w'$ ist bereits
%     Element einer Lösung mit $|w'| < |u_1\dots u_n w'| = |w|$. Widerspruch zur minimalen Länge von $w$.
%  \end{itemize}
...
\begin{proof}[Kleene, \=>]
  ...
  Mit Hilfe von Ardens Lemma (und weiteren Rechenregeln für Sprachen) können wir nun das Gleichungssystem (\ref{eq:constraintsForRegex}) iterativ lösen.
    ...
    \begin{align*}
      ...
      \intertext{(Distributivgesetz: $R_{in}R_{nn}^*$)}
