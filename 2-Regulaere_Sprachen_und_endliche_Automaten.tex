...
%     \end{itemize}
% \end{Bsp}
\begin{Def}[name={[Erweiterung von $\delta$ auf Worte]}]\label{def:2.deltaschlange}
        Die Erweiterung von $\delta:Q\x\Sigma\->Q$ auf Worte $\tilde{\delta}: Q\x\Sigma^*\->Q$ ist induktiv definiert durch
  \begin{enumerate}
  \item $\tilde\delta(q,\Eps) =q$ (Wortende erreicht)
  \item $\tilde\delta(q,aw)=\tilde\delta(\delta(q,a),w)$ (Rest im Folgezustand verarbeiten)
  \end{enumerate}
\end{Def}
\begin{Def}[name={[Die durch einen \acs*{DEA} erkannte Sprache]}]\label{def:2.sprache}
        Sei $\A=(\Sigma, Q,\delta,\qinit,F)$. Die \emph{von $\A$ erkannte Sprache} ist
...
\begin{align*}
  \tilde{\delta}_\cap((q_1,q_2),aw) 
    & \stackrel{\autoref{def:2.deltaschlange}}{=} \tilde{\delta}_\cap(\delta_\cap((q_1,q_2),a),w)\\
...
\datenote{25.10.17}
Sei $A_1$ der folgende \ac{DEA} über dem Alphabet $\Sigma=\{0,1\}$ dessen graphische Repräsentation nahezu mit $A_{\mathtt{Macaron}}$ identisch ist.
...
\begin{Def}
  Ein Zustand $q \in Q$ heißt \emph{erreichbar}, falls ein $w \in \Sigma^*$ existiert, so dass $\hat \delta(q_0, w) = q$.
...
Die Zustände $q_{11}$, $q_{20}$ und $q_{21}$ aus \autoref{bsp:2.schnitt} sind paarweise äquivalent.
...
Bedenken: Jeder Zustand hat für jedes Zeichen einen Nachfolger. Wenn wir Zustände verschmelzen gibt es mehrere Nachfolger!
...
\begin{align*}
        p \equiv q  
        & \<=> \forall w\in\Sigma^*: \tilde\delta(p,w)\in F \<=> \tilde\delta(q,w) \in F\\
        & \<=> (p\in F \<=> q \in F) \land \forall a\in \Sigma: \forall w\in\Sigma^*:
        \tilde\delta(p,aw)\in F \<=> \tilde\delta(q,aw)\in F\\
        &\=>  \forall a\in\Sigma: \forall w\in\Sigma^*: \tilde\delta(\delta(p,a),w)\in F \<=> \tilde\delta(\delta(q,a),w)\in F\\
...
\begin{Def}[name={[Äquivalenzklassenautomat]}]
        Der Äquivalenzklassenautomat $\A'=(Q',\Sigma,\delta',{\qinit}',F')$ zu einem DEA $\A=(Q,\Sigma,\delta,\qinit,F)$ ist bestimmt durch:
        \begin{align*}
...
\begin{Satz}[name={[Äquivalenzklassenautomat ist wohldefiniert]}]
        Der Äquivalenzklassenautomat ist wohldefiniert und $L(\A')=L(\A)$.
\end{Satz}
\begin{proof}\ 
        \begin{enumerate}
                \item Wohldefiniert: zu zeigen $\delta'([q],a) =[\delta(q,a)]$ ist nicht abhängig von der Wahl des Repräsentanten $q\in [q]$. Das folgt direkt aus \autoref{eqn:delta-wohldefiniert} gezeigt.
                \item $L(\A)=L(\A')$ zeige für alle $w\in\Sigma^*$ und alle $q$: $\tilde\delta(q,w)\in F \<=> \tilde\delta'([q],w)\in F'$\\
                Induktion über $w$:\\
                I.A. $w=\Eps$: $\tilde\delta(q,\Eps)=q\in F \<=> \tilde\delta'([q],\Eps)=[q]\in F'$ nach Definition.\\
                I.V.: $\forall w'\in\Sigma$, $\forall q\in Q$, $\tilde\delta(q,w')\in F \<=> \tilde\delta'([q],w')\in F'$\\
                I.S.: \begin{align*}
                \tilde\delta(q,aw')\in F &\<==> \tilde\delta(\delta(q,a),w')\in F\\ &\xLeftrightarrow{I.V.} \tilde\delta'([\delta(q,a)],w')\in F'\\
                &\<==> \tilde\delta'(\delta'([q],a),w')\in F'\\
                &\<==> \tilde\delta'([q],a w')\in F'
                \end{align*}
                Also f"ur $q=q_0$: $\forall w\in\Sigma^*$, $w\in L(\A)\<==> \tilde\delta(q_0,w) \in F \<=> \tilde\delta'([q_0],w) \in F' \<==> w\in L(\A')$
        \end{enumerate}
\end{proof}
\emph{Bem:} Die Konstruktion von $\A'$ kann in $O(|Q||\Sigma|\log|Q|)$ passieren.
...
\begin{Bsp} %\rlnote{Bsp.-Num. überprüfen (3.6)}
  \label{Bsp:R_m}
        ...
        \[ R_\A = \{(u,v) \mid \tilde\delta(q_0,u)=\tilde\delta(q_0,v)\} \]
        \begin{itemize}
...
\begin{proof}
  ...
  \begin{displaymath}
    L(\A)=\{w \mid \tilde\delta(q_0,w)\in F \} = \bigcup_{q \in F} \{ w \mid \tilde\delta(q_0,w) = q\}
\end{displaymath}

Nun sind $\{ w \mid \tilde\delta(q_0,w)=q \} = [q]_\A$ genau die Äquivalenzklassen von $R_\A$ aus Bsp \ref{Bsp:R_m}, einer rechtsinvarianten Äquivalenzrelation.
...
\vspace{-1em}
\begin{proof}
	Sei $\A=(\Sigma, Q,\delta,q_0,F)$ ein \ac{DEA} für $L$.\\
...
\begin{Def}[name={[NEA]}]
	Ein \ac{NEA} (\acsu{NEA} = \acl{NEA}) $\A = (\Sigma, Q,\delta,q_0,F)$ mit
	\begin{itemize}
