...
\begin{Bsp}
TODO: Teilworterkennung oder n-letztes
\end{Bsp}

Bemerkung: TODO Wortproblem schwieriger


Bemerkung: Die \ac{NEA} und \ac{DEA} Definitionen in der Literatur sind nicht einheitlich. 
...
Alternativ könnte man auch zunächst den \ac{NEA} einführen und den \ac{DEA} als Spezialfall dessen definieren.
...
\begin{proof}[Potenzmengenkonstruktion]
        ...
        TODO: Beispiel 2-letztes
        
        Wir wollen nun $L(\A_\calP)=L(\calN)$ zeigen. 
...
\begin{Def}
 Der \emph{$\Eps$-Abschluss} $\EAbs:Q\rightarrow\calP$ ist die kleinste Abbildung, die die folgenden Eigenschaften erfüllt:
$$\forall q\in Q: \quad q\in\EAbs(q)$$
$$\forall q,q',q''\in Q: \quad q'\in\EAbs(q) \text{ und } q''\in\delta(q',\Eps) \quad\Rightarrow\quad q''\in\EAbs(q)$$
...
$$\forall q,q'\in Q: q'\in\EAbs(q) \quad\Rightarrow\quad (q,\Eps,q')\in\reach$$
$$\forall q,q',q'',q'''\in Q, w\in\Sigma^*: \quad q'\in\EAbs(q), q''\in\delta(q',a) \text{ und } (q'',w,q''')\in\reach \Rightarrow (q,aw,q''')\in\reach$$
\end{Def}
...
\begin{proof}[$\Eps$-Eliminierung]
        ...
        $$\delta_\calN(q,a)=\bigcup\limits_{q'\in\EAbs}\{p'\mid \exists p : p'\in\EAbs(p) \text{ und } p\in\delta(q',a)\}$$
        $$F_\calN=\bigcup\limits_{q_f\in F}\EAbs(q_f)$$
