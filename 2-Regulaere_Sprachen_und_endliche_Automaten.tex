...
\begin{proof}\ 
        ...
        \end{enumerate}
\end{proof}
In den Übungen werden wir ein Verfahren mit $O(|Q|^4\cdot |\Sigma|\log|Q|)$ Laufzeit zur Konstruktion des Äquivalenzklassenautomaten kennenlernen. Es gibt aber auch schnellere Verfahren. Mit dem Algorithmus von Hopcroft kann $\A_\equiv$ in $O(|Q||\Sigma|\log|Q|)$ erzeugt werden.
...
\begin{proof}
        ...
        Für beliebigen \acs*{DEA} $A'$ ist $|Q|$ also nie größer als $|Q'|$.
\end{proof}
...
Naiver Lösungsversuch:
	...
	\end{figure}
Problem: Diagramm beschreibt keinen \acs*{DEA}. Startzustand hat zwei ausgehende Kanten für $1$.
...
Es gibt also mindestens $2^n$ Äquivalenzklassen und aus \autoref{kor:2.minAutomat} wissen wir dass ein minimaler \acs*{DEA}, der $L_n$ akzeptiert $2^n$ Zustände hat.
...
dabei ist
        ...
        \end{itemize}
\end{Def}
Bemerkung: Die Definition des \acs*{NEA} unterscheidet sich vom \acs*{DEA} also nur in der Transitionsfunktion. Beim \acs*{DEA} ist der Bildbereich der Transitionsfunktion die Menge der Zustände $Q$. Hier ist der Bildbereich die Potenzmenge der Zustandsmenge $\mathcal{P}(Q)$.
Analog zu \acs*{DEA}s werden wir auch \acs*{NEA}s mit Hilfe eines Zustandsdiagramms beschrieben. Z.B. beschreibt \autoref{fig:2.nletztesNea} für jedes $n\in\N$ einen \acs*{NEA} für die Sprache $L_n$.
...
Ein sinvolles Vorgehen scheint jeden initialen Lauf zu betrachten, doch z.B. für das Wort $111$ hat obiger \acs*{NEA} bereits  drei verschiedene initiale Läufe: $q_0q_0q_0$, $q_0q_0q_1$, $q_0q_0q_2$.
...
\end{Def}
\begin{Bsp}
Der Potzenzmengenautomat für den \acs*{NEA} aus \autoref{bsp:2.zweitletztes} hat das folgende Zutandsdiagramm.
  \begin{center}
                \begin{tikzpicture}[circle/.style={
                        shape=circle,
                        minimum size=0.5cm,
...
Idee: Geben \acs*{DEA} $\A_1$ mit $L(\A_1)=L_1$ und \acs*{DEA} $\A_2$ mit $L(\A_2)=L_2$,
...
Links: \acs*{DEA} $\A_1$, der Automat aus \autoref{bsp:2.zweitletztes} eingeschränkt auf die erreichbaren Zustände. Rechts: \acs*{DEA} $\A_2$ dessen Sprache $\{w\in\{0,1\}^*\mid \text{Anzahl $1$ in $w$ ist ungerade}\}$ ist.
  \begin{center}
                \begin{tikzpicture}[circle/.style={
                        shape=circle,
                        minimum size=0.5cm,
                ...
                \end{tikzpicture}
  \end{center}
Unten: \acs*{NEA} $\calN_\mathsf{naiv}$, Resultat einer einer naiven und inkorrekten Konstuktion für die Konkatenation.
  \begin{center}
                \begin{tikzpicture}[circle/.style={
                        shape=circle,
                        minimum size=0.5cm,
                ...
                \end{tikzpicture}
  \end{center}
Dieser \acs*{NEA} akzeptiert nun auch das Wort $w=11011$. Allerdings ist $w$ nicht in der Konkatenation $L(\A_1)\cdot L(\A_2)$ denn es gibt keine Zerlegung $w=w_1\cdot w_2$ bei der sowohl der Prefix $w_1$ von $\A_1$ als auch der Suffix $w_2$ von $\A_2$ akzeptiert wird.
\end{Bsp}
...
wobei $\Sigma$, $Q$, $\qinit$, $F$ wie bei \acs*{NEA}s (bzw. \acs*{DEA}s) definiert sind und die Transitionsfunkion den folgenden Typ hat.
...
\begin{Def}
 ...
 \begin{eqnarray*}
  & q\in\ecl_\B(q) &\\
  & q'\in\ecl_\B(q) \text{ und } q''\in\delta(q',\Eps) \quad\Rightarrow\quad q'''\in\ecl_\B(q) & \qedhere
 \end{eqnarray*}
\end{Def}
...
& q'\in\ecl_\B(q), q''\in\delta(q',a) \text{ und } (q'',w,q''')\in\reach_\B \Rightarrow (q,aw,q''')\in\reach_\B & \qedhere
 \end{eqnarray*}
\end{Def}
...
$$\exists \text{ Lauf } q_0q_1\ldots q_n \text{ von $\calN$ über } w \quad\Leftrightarrow\quad (q_0,w,q_n)\in\reach_\B$$
...
\begin{Def}[$\Eps$-freier Automat]
 Für gegebenen $\Eps$-\ac{NEA}  $\B= (\Sigma, Q,\delta,\qinit,F)$ definieren wir den \acs*{NEA} $\calN= (\Sigma, Q,\delta_\calN,\qinit,F_\calN)$ mit
...
%         \end{eqnarray*}
        ...
        \begin{align*}
         w\in L(\B) 
         &\stackrel{\autoref{def:2.sprache}}{\<=>} \exists q_f\in F: (\qinit, w, q_f)\in\reach_\B\\
