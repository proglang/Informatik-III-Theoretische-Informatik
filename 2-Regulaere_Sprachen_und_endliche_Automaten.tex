...
Im folgenden schreiben wir $REG$ für die Menge alles regulären Sprachen.
...
Wir konstruieren den DEA $\overline{\A}=(\Sigma, Q, \delta, \qinit, Q\backslash F)$ bei dem ein Zustand akzeptierend ist genau dann wenn der Zustand in $\A$ nicht akzeptierend ist.
Man kann leicht zeigen, dass $L(\overline{\A})=\overline{L}$ und somit ist auch $\overline{L}$ regulär.
\end{proof}
...
Wir konstruieren einen $\Eps$-NEA $\B_*=(\Sigma, Q, \delta_*, \qinit, F)$ für $L^*$ indem wir von jedem akzeptierenden Zustand einen $\Eps$-Übergang zum Startzustand hinzufügen.
\[
                \delta(q,x) =
                        \begin{cases}
                                \delta(q,x) & q\notin F \text{ oder } x \neq \Eps\\
...
\begin{tabular}{cl@{\quad}l}
 ...
 $\phantom{X}^*$ & (Kleene Stern) & \autoref{satz:2.ClosedStar} 
\end{tabular}
