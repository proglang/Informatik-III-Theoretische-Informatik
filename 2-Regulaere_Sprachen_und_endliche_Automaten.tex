...
% \end{Bsp}
\begin{Def}[\acs*{DEA}]
	Ein \emph{\acf{DEA}}, (\acsu{DEA} $\hat=$ \acl{DEA}) ist ein 5-Tupel
...
\begin{Def}[name={[Äquivalenz von \acs*{DEA}-Zuständen]}] %\rlnote{Def.-Num. überprüfen}
	Zwei Zustände $q,p\in Q$ eines \ac{DEA} sind \emph{äquivalent}, geschrieben $p\equiv q$, falls $\forall w\in\Sigma^*$, $\hat\delta(p,w)\in F \<=> \hat\delta(q,w)\in F$
\end{Def}
%\stepcounter{lemma}
