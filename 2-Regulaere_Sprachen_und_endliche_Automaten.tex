...
\begin{Satz}[Rabin und Scott]\label{satz:2.rabinscott}
	Zu jedem \ac{NEA} $\calN$ mit $n$ Zuständen gibt es einen \ac{DEA} $\A_\calP$ mit $2^n$ Zuständen, sodass $L(\A_\calP)=L(\calN)$.
\end{Satz}
...
\begin{proof}[von \autoref{satz:2.rabinscott}]
        ...
        \begin{eqnarray*}
                ...
                \hspace*{-2.5mm}& \Leftrightarrow & q\in\tilde\delta_\calP(\delta_\calP(p,a),w')\\
...
Unten: \acs*{NEA} $\calN_\mathsf{naiv}$, Resultat einer einer naiven und inkorrekten Konstuktion für die Konkatenation.
  \begin{center}
                ...
                \end{tikzpicture}
  \end{center}
Dieser \acs*{NEA} akzeptiert nun auch das Wort $w=11011$. Allerdings ist $w$ nicht in der Konkatenation $L(\A_1)\cdot L(\A_2)$ denn es gibt keine Zerlegung $w=w_1\cdot w_2$ bei der sowohl der Präfix $w_1$ von $\A_1$ als auch der Suffix $w_2$ von $\A_2$ akzeptiert wird.
\end{Bsp}
...
\begin{Satz}\label{satz:2.EpsElim}
    Zu jedem $\Eps$-NEA $\B$ gibt es einen \ac{NEA} $\calN$, sodass $L(\calN)=L(\B)$.
\end{Satz}
Zur Vorbereitung des Beweises machen wir zunächst die folgende Definition.
